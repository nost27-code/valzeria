...
class EquipmentMarketService
{
    ...
    public function listEquipment(Character $seller, CharacterItem $characterItem, int $listingPrice): EquipmentMarketListing
    {
        return DB::transaction(function () use ($seller, $characterItem, $listingPrice) {
            ...
            $listing = EquipmentMarketListing::create([
                ...
                'weapon_category' => $this->equipmentCategory($item),
                'weapon_rank' => $this->equipmentRank($item),
                ...
                'body_appraisal_price' => $appraisal['body_appraisal_price'],
                'trait_appraisal_price' => $appraisal['trait_appraisal_price'],
                'appraisal_price' => $appraisal['appraisal_price'],
                'minimum_price' => $appraisal['minimum_price'],
                'maximum_price' => $appraisal['maximum_price'],
                'listing_price' => $listingPrice,
                'appraisal_version' => $appraisal['appraisal_version'],
    ...
    private function assertMarketListable(CharacterItem $item): void
    {
        ...
        if ($item->is_locked) throw new RuntimeException('保護中の装備品は出品できません。');
        if ($item->isMarketListed()) throw new RuntimeException('この装備はすでに出品中です。');
        if (! (bool) $item->is_tradeable || ! (bool) ($item->item?->is_tradeable ?? true)) throw new RuntimeException('この装備は取引できません。');
        if ($item->market_relistable_at?->isFuture()) throw new RuntimeException('この装備は市場で購入したばかりです。再出品可能日時：' . $item->market_relistable_at->format('Y年n月j日 H:i'));
    }

    private function assertOwnedBy(CharacterItem $item, Character $seller): void
    {
        if ((int) $item->character_id !== (int) $seller->id) throw new RuntimeException('この装備は所持していません。');
    }
    ...
    private function assertListingLimit(Character $seller): void
    {
        if (EquipmentMarketListing::query()->where('seller_character_id', $seller->id)->where('status', 'active')->where('expires_at', '>', now())->count() >= (int) config('equipment_market.max_active_listings')) {
            throw new RuntimeException('同時に出品できる装備は' . config('equipment_market.max_active_listings') . '件までです。');
        }
    }
    ...
    private function makeSnapshot(CharacterItem $item): array
    {
        ...
        return [
            ...
            'icon_path' => $item->item->iconImagePath(),
            'equipment_category' => $this->equipmentCategory($item), 'equipment_rank' => $this->equipmentRank($item),
            'weapon_category' => $this->equipmentCategory($item), 'weapon_rank' => $this->equipmentRank($item),
    ...
    {
        $this->notificationService->create($seller, 'market', 'equipment_market_sold', '【装備市場】装備が売れました', "{$listing->display_name_snapshot}\n販売価格：" . number_format($price) . "G\n成立手数料：" . number_format($fee) . "G\n受取Gold：" . number_format($proceeds) . 'G', '履歴を見る', route('equipment-market.index', ['tab' => 'history']), ['equipment_market_listing_id' => $listing->id], 70, now()->addDays(7));
    }

    private function notifyPurchase(Character $buyer, EquipmentMarketListing $listing, int $price): void
    {
        $this->notificationService->create($buyer, 'market', 'equipment_market_purchased', '【装備市場】装備を購入しました', "{$listing->display_name_snapshot}\n購入価格：" . number_format($price) . 'G\n装備画面または持ち物から確認できます。', '装備を見る', route('equipment.index'), ['equipment_market_listing_id' => $listing->id], 70, now()->addDays(7));
    }

    private function equipmentCategory(CharacterItem $item): ?string
    {
        return $item->item?->type === 'armor'
            ? $item->item?->armor_category
            : $item->item?->weapon_category;
    }

    private function equipmentRank(CharacterItem $item): string
    {
        return strtoupper((string) ($item->item?->weapon_rank ?: $item->item?->armor_rank ?: ''));
    }
}
