...
class ArenaNpcBattleService
{
    ...
    public function __construct(
        ...
        private readonly ?DamageApplicationService $configuredDamageApplicationService = null,
    ) {
    }
    ...
    private function makePlayerActor(Character $character): BattleActor
    {
        ...
        $this->jobArtBattleSupport->attachBossSet($actor, $character, 'champ', 'arena_npc', true);
    ...
    private function executeSkillAction(
        ...
        for ($i = 0; $applyTargetEffects && $i < $hitCount; $i++) {
            $damage = 0;
            $powerCenti = $totalPowerCenti === null ? null : $hitPowers[$i];
            $power = $powerCenti === null
                ? $hitPowers[$i]
                : intdiv($powerCenti + 50, 100);
        ...
        if (!$skill->isJobArt() && (int) $skill->self_buff_percent > 0) {
            ...
            $state->addLog("{$attacker->name} の攻撃と魔力が上昇した！");
        }
    }
    ...
    private function normalAttackType(?object $job): string
    {
        ...
        return in_array($type, ['physical', 'magical', 'adaptive'], true) ? $type : 'physical';
    }
