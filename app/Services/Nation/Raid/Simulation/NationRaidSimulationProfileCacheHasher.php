...
/** random_int()を含むaction profile cacheを内容hash付きの監査証跡へ固定する。 */
final class NationRaidSimulationProfileCacheHasher
{
    ...
    public function sealProfiles(array $profiles): array
    {
        foreach ($profiles as &$profile) {
            if (! is_array($profile)) {
                throw new InvalidArgumentException('Action profile must be an array.');
            }
    ...
    public function rootCacheHash(
        ...
        foreach ($characters as $character) {
            $entries[] = [
                ...
                'cache_hash' => $character['action_profile_cache_hash'] ?? null,
            ];
        }
        ...
        return $this->hash([
            ...
            'action_profile_model' => $modelVersion,
            'characters' => $entries,
        ]);
    }
