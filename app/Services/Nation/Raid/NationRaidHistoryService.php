...
final class NationRaidHistoryService
{
    ...
    public function forCharacter(Character $character): array
    {
        ...
        $history = NationRaidEvent::query()->where('status', NationRaidEvent::STATUS_COMPLETED)
            ...
            ->select(['id', 'name', 'status', 'starts_at', 'ends_at', 'result_type', 'result_progress_bps', 'final_standings_hash'])
        ...
        $history->through(function (NationRaidEvent $event) use ($character): array {
            ...
            return [
                ...
                'outcome_label' => match ($event->result_type) {
        ...
        $pending = NationRaidPersonalReward::query()->where('account_id_snapshot', $character->user_id)
            ...
            ->where(function (Builder $query): void {
                $query->whereHas('event', fn (Builder $event) => $event->where('status', NationRaidEvent::STATUS_COMPLETED))
                    ->orWhere(function (Builder $reward): void {
                        $reward->where('availability_type', NationRaidPersonalRewardCatalog::AVAILABILITY_IMMEDIATE)
