...
class MapExplorationBatchService
{
    ...
    private function executeOne(Character $character, MapExplorationBatch $batch, ExplorationMap $map, string $root, int $index): array
    {
        ...
        return [
            ...
            'job_art_activation_attempts' => $battle->jobArtActivationAttempts,
            'job_art_loadout' => $battle->jobArtLoadout,
            'character_level_at_start' => $battle->playerLevelAtStart,
            'current_job_id_at_start' => $battle->playerJobIdAtStart,
            'log' => $logText,
            'enemy' => $enemy,
    ...
    private function presentBattleResult(MapExplorationBatch $batch, array $lastResult, array $runs, ?string $stopReason = null): array
    {
        ...
        foreach ($runs as $offset => $run) {
            ...
            if (is_array($run['map_drop'] ?? null)) {
                $mapDrops[] = $run['map_drop'];
            }
            ...
            $displayRuns[] = [
                ...
                'turn_count' => (int) ($run['turn_count'] ?? 0),
                'job_art_usage' => (array) ($run['job_art_usage'] ?? []),
                'job_art_activation_attempts' => (array) ($run['job_art_activation_attempts'] ?? []),
                'job_art_loadout' => (array) ($run['job_art_loadout'] ?? []),
                'character_level_at_start' => $run['character_level_at_start'] ?? null,
                'current_job_id_at_start' => $run['current_job_id_at_start'] ?? null,
        ...
        $stopReason ??= in_array($lastOutcome, ['victory', 'win'], true)
            ? ($completed < (int) $batch->requested_count ? 'map_availability_exhausted' : 'completed')
