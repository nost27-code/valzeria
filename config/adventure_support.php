<?php

return [
    'items' => [
        'adventurer_departure_set' => [
            'name' => '冒険者旅立ちセット',
            'category' => 'おすすめ',
            'icon_image' => 'images/icon/icon_259.webp',
            'price' => 100,
            'description' => '冒険に役立つ支援パス・探索力の薬・倉庫拡張と、限定カードフレームをまとめた一度限りのセットです。',
            'effect_type' => 'adventurer_departure_set',
            'purchase_limit' => 1,
            'home_banner_until_city_id' => 4,
            'purchase_button_label' => '旅立ちセットを購入',
            'regular_value_kiseki' => 225,
            'approx_value_jpy' => 980,
            'requires_support_pass' => true,
            'effects' => [
                '冒険者支援パス30日利用券 ×1（通常50輝石 / 所持品から使用）',
                '探索力の薬 ×3（通常75輝石）',
                '素材倉庫拡張 ×1（通常50輝石 / 保管枠+500）',
                '装備倉庫拡張 ×1（通常50輝石 / 保管枠+300）',
                '限定冒険者カードフレーム（外枠・丸枠）',
            ],
            'grants' => [
                'consumables' => [
                    'support_pass_30d_ticket' => 1,
                    'explore_stamina_potion' => 3,
                ],
                'storage_expansions' => [
                    'material_storage_expand' => 1,
                    'equipment_storage_expand' => 1,
                ],
                'profile_assets' => [
                    [
                        'asset_type' => 'card_frame',
                        'asset_path' => 'images/profile/adventurer_card_frame91.webp',
                    ],
                    [
                        'asset_type' => 'avatar_frame',
                        'asset_path' => 'images/profile/adventurer_avatar_frame91.webp',
                    ],
                ],
            ],
        ],
        'explore_stamina_small_bottle' => [
            'name' => '探索力の小瓶',
            'category' => '探索支援',
            'icon_image' => 'images/icon/icon_088.webp',
            'price' => 10,
            'description' => '使用すると探索力を50回復します。回復分は上限を超えて蓄積できます。',
            'effect_type' => 'explore_stamina_recovery',
            'effect_value' => 50,
            'daily_purchase_limit' => 10,
        ],
        'explore_stamina_potion' => [
            'name' => '探索力の薬',
            'category' => '探索支援',
            'icon_image' => 'images/icon/icon_089.webp',
            'price' => 25,
            'description' => '使用すると探索力を150回復します。回復分は上限を超えて蓄積できます。',
            'effect_type' => 'explore_stamina_recovery',
            'effect_value' => 150,
            'daily_purchase_limit' => 10,
        ],
        'support_pass_30d' => [
            'name' => '冒険者支援パス 30日',
            'category' => '冒険者支援パス',
            'icon_image' => 'images/icon/icon_259.webp',
            'price' => 50,
            'description' => '購入すると30日利用券が所持品に入ります。利用券を使用すると、探索力上限が+250され、冒険者カードの特別な見た目を30日間選べるようになります。強さに直接影響する効果はありません。',
            'effect_type' => 'support_pass_30d',
            'effects' => [
                '探索力上限 +250',
                '冒険者カード見た目「支援パスカード」解放',
                '所持品から使用後、有効期間：30日',
                '自動更新なし',
            ],
        ],
        'silver_week_extension_pass_30d' => [
            'name' => 'シルバーウィーク仕様延長パス 30日',
            'category' => '冒険者支援パス',
            'icon_image' => 'images/icon/silver_week_extension_pass.webp',
            'price' => 105,
            'description' => '購入すると30日利用券が所持品に入ります。利用券を使用すると、探索力が45秒で1回復し、上限が+500されます。通常の冒険者支援パスとは独立して利用できます。',
            'effect_type' => 'silver_week_extension_pass_30d',
            'effects' => [
                '探索力の自然回復：45秒で1',
                '探索力上限 +500',
                '通常の冒険者支援パスと併用可能',
                '所持品から使用後、有効期間：30日',
                '自動更新なし',
            ],
        ],
        'adventurer_supply_box' => [
            'name' => '冒険者補給箱',
            'category' => '探索支援',
            'icon_image' => 'images/icon/icon_087.webp',
            'price' => 10,
            'description' => '薬草・回復薬・魔力水を各10個補充します。探索に持ち込める数は、これまで通り各10個までです。',
            'daily_purchase_limit' => 1,
        ],
        'material_storage_expand' => [
            'name' => '素材倉庫拡張',
            'category' => '保管拡張',
            'icon_image' => 'images/icon/icon_085.webp',
            'price' => 50,
            'description' => '素材の保管枠を+500します。進化素材や秘境素材を多く保管したい冒険者向けです。',
            'effect_value' => 500,
            'purchase_limit' => 30,
        ],
        'material_storage_gold_expand' => [
            'name' => '素材倉庫Gold拡張',
            'category' => '保管拡張',
            'icon_image' => 'images/icon/icon_085.webp',
            'currency' => 'gold',
            'price' => 100000,
            'description' => 'Goldを支払って素材の保管枠を+50します。少しずつ倉庫を広げたい冒険者向けです。',
            'effect_value' => 50,
            'purchase_limit' => 10,
        ],
        'equipment_storage_expand' => [
            'name' => '装備倉庫拡張',
            'category' => '保管拡張',
            'icon_image' => 'images/icon/icon_086.webp',
            'price' => 50,
            'description' => '武器・防具・装飾品の保管枠を+300します。分岐装備やレア装備を集めたい冒険者向けです。',
            'effect_value' => 300,
            'purchase_limit' => 20,
        ],
        'rescue_insurance' => [
            'name' => '救助保険証',
            'category' => '救助',
            'icon_image' => 'images/icon/icon_090.webp',
            'price' => 10,
            'description' => '探索開始前に使うと、全滅時の入手品ロストを25%に抑えます。危険な連戦に挑む前の備えです。',
            'sale_suspended' => true,
        ],
        'emergency_rescue_request' => [
            'name' => '緊急救助要請',
            'category' => '救助',
            'icon_image' => 'images/icon/icon_091.webp',
            'price' => 50,
            'description' => '全滅時に使用すると、今回の入手品ロストを0にします。ただし、探索は終了し、街へ帰還します。',
            'daily_use_limit' => 1,
            'sale_suspended' => true,
        ],
    ],
    'inventory_items' => [
        'experience_talisman' => [
            'name' => '経験の護符',
            'category' => '経験支援',
            'icon_image' => 'images/icon/icon_014.webp',
            'description' => '使用すると、通常探索の次の50勝で獲得経験値が25%増加します。ボス戦など対象外の戦闘や敗北では残り回数を消費しません。',
            'effect_type' => 'normal_exploration_exp_boost',
            'effect_value' => 25,
            'eligible_victories' => 50,
            'is_tradable' => false,
        ],
        'support_pass_30d_ticket' => [
            'name' => '冒険者支援パス30日利用券',
            'category' => '支援アイテム',
            'icon_image' => 'images/icon/icon_259.webp',
            'description' => '使用すると冒険者支援パスが30日間有効になります。残り期間がある場合は現在の期限から30日延長されます。',
            'effect_type' => 'support_pass_activation',
            'effect_value' => 30,
        ],
        'silver_week_extension_pass_30d_ticket' => [
            'name' => 'シルバーウィーク仕様延長パス30日利用券',
            'category' => '支援アイテム',
            'icon_image' => 'images/icon/silver_week_extension_pass.webp',
            'description' => '使用するとシルバーウィーク仕様延長パスが30日間有効になります。残り期間がある場合は現在の期限から30日延長されます。',
            'effect_type' => 'silver_week_extension_pass_activation',
            'effect_value' => 30,
        ],
    ],
];
