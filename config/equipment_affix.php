...
return [
    ...
    // 調律のHPは防具だけ6倍にして、新しいPvE防御式での一撃耐えを補助する。
    'armor_tuning_hp_multiplier' => 6.0,
    ...
    // 武器固有特攻と個体特攻が同じ種族へ重なった場合の合計上限。
    'weapon_killer_damage_rate_cap' => 0.55,

    // 防具の種族耐性レート（段階I〜V、品質倍率を掛ける前）。
    // 敵→プレイヤーの直接ダメージ計算式（減算式/割合式）に依存せず、最終ダメージへ乗算する。
    'armor_resist_rates' => [
        1 => 0.05,
        2 => 0.10,
        3 => 0.15,
    ...
    // 戦闘時に適用する種族耐性の上限（逸品V=27%を収める）。
    'armor_resist_damage_reduction_cap' => 0.35,
