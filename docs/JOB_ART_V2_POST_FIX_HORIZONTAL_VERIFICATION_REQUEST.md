...
1. 「竜冠天穿槍」が任意のRank5連携使用後に470%となり、後段のCrownBalance再適用後も維持される
...
4. 封式の場による基礎効果だけの発動でも、旧DB master値ではなくL列正本値を使う
5. 同種の静的上書き、二重適用、適用漏れ、正本混入が、ほかの戦技に残っていない
...
- Rank5の所属系譜、現在職／継承、進行metadataの有無、HIT／MISS／EVADEを問わず使用済みへ記録すること
- feature flag OFFでは使用済みを記録せず、355%の旧経路へfail closedすること
...
対人系の封式の場で固有追加効果を抑止しても、実行用`Skill`の基礎値は`JobArtV2CrownBalanceCatalog`のL列正本値へ合わせることが期待値です。

- 対象経路: PvP、チャンプ戦、NPC闘技場
- 旧DB master値を基礎値として復活させない
- role／progression／prepared／fieldなど、抑止対象の動的固有効果は再適用しない
- source masterは変更しない
- 代表値: `66:9 聖冠アイギスロード`は威力355%・軽減45%、`16:9 傭兵団の総攻撃`は威力255%・持続4・防御／精神低下20%
...
- 未確認件数
- 影響する戦闘経路
- 今回までの修正点、封式の場の正本値、竜冠355%／470%回帰の合否
...
- [ ] `job_arts.json`のRank5全94件が、結果・系譜・metadataの有無を問わず470%条件を成立させることを確認した
- [ ] 封式の場でRank9基礎効果がL列正本値となり、動的固有効果は抑止されたままであることを確認した
- [ ] 「確定不具合」「要裁定」「未確認」を分離した
