...
2026-09-06の開始後、第1〜5再臨が約15分で撃破された実績を受け、初回の第1〜8再臨は各1000万/2000万のまま、第9〜12再臨を各2億、第13〜16再臨を各5億、第17〜20再臨を各10億へ増加する裁定を得た。総HPは69億2000万。専用CLIは対象初回event・旧/new ruleset hash・管理者・承認根拠を照合し、coordinator/event/cycleをlockして変更前snapshotをprivate storageへ保存する。完了済み個体を変更せず、現在個体が対象段階なら既与damageを維持する。Seeder・migration・既存プレイヤーデータの一括補正は行わない。
...
MariaDB 10.5.13の隔離CIは新5テーブルをInnoDB必須対象に含め、基本無料→自主出撃と同種返却、既存の並行精算・報酬競合を確認する。SQLiteのFeature testだけを本番DB互換の証明にしない。本番公開は別承認とし、公開時は同一SHAのstaging→production、pending migration、event ruleset/reward hash、毎分scheduler、画面、匿名境界、ログ差分を読み戻す。
