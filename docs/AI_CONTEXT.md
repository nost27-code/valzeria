...
- 国家対抗レイド初回（2026-09-06）: 正式TOP・20再臨の戦闘・順位・履歴・報酬受取と管理者運用を実装。正式公開は`NATION_COMPETITIVE_RAID_ENABLED`（既定OFF）、案内は独立した`NATION_COMPETITIVE_RAID_PREVIEW_ENABLED`。開催は承認済みeventの状態で管理し、入口で自動作成しない。初回は探索力10・回数上限なし・7日間。HPは第1〜4再臨各1000万、第5〜8各2000万、第9〜12各2億、第13〜16各5億、第17〜20各10億、総HP69億2000万。共闘ボーナスは2人+3%から22人以上+22%まで。切替前に受付済みの出撃は受付時の倍率を使い、初回専用の予告短縮・HP曲線・共闘段階CLIを持つ。初回報酬は終了後に明示確定して本人受取とし、次回用rulesetを遡及しない。
- 国家対抗レイド次回以降（2026-09-16実装・開催値承認）: 次回は2026-09-25 09:00〜2026-10-02 09:00（日本時間）。ボスは機械種族の「十系を模す天墜機神 アストラギア」で、封殻・展装・十系演算・露核の4形態を持つ。主再臨HPは1〜4各3,000万、5〜8各5,000万、9〜12各1億、13〜16各2億、17〜20各5億、総HP35億2,000万。称号「天墜機神を穿つ者」「天墜機神討滅の功臣」、国家討旗、討滅参加記録はアストラギア固有名を使い、過去のヴァルグレイド開催は旧名称・竜種族・画像・技名・報酬名・HPと保存済みhashを維持する。開始前はプレイヤー向けの名称・画像・種族・形態・技名・固有称号名を伏せて「次回レイド準備中」とし、`starts_at`到達後に開催snapshotを自動公開する。開催は7日間。基本無料出撃は1日3回分、繰越上限9回分で先に消費し、その後は探索力10で回数上限なく自主出撃できる。開催72時間前に、直近7日で実戦した国家所属者を基準人数Nとして固定し、各人の1日最初の通常探索勝利を最大3点記録する。準備度は`floor(貢献/(2N)*100)`上限100%で、50%以上は無料4回/日、80%以上は繰越12回、100%は侵攻被害-10。未復興国は上乗せだけを保留する。全体HP削減率100%は討滅、80%以上は撃退、80%未満は侵攻。侵攻基礎被害は進行60%以上20、30%以上40、30%未満60で、準備度80/100%と有効参加率50/75%（有効出撃5回以上）による各-5/-10、下限10。復興目標は`ceil(N×最終被害/10)`、通常探索勝利で1人1日3点まで進む。参加5回・個人ダメージ・全体到達の順位非依存報酬は条件成立時に受取権利を固定し、順位・国家報酬は終了30分後を目安に自動確定する。未確定出撃等が残れば毎分再試行する。管理画面は参加率、無料/自主出撃、探索力消費、上位damage集中率を開催単位で集計する。2026-09-16にSHA`e624dcdb179ebb69e98c9707cb80f700480afee8`をstaging→productionへ同一SHA・`migration_mode=backward_compatible`で配備し、本番MariaDB 10.5.26でmigration batch 327、予約中event 0件、新規5テーブル0件、既存v7 eventと報酬の非遡及を読み戻した。アストラギア差し替えは予約済み開催へ保護付きCLIで開戦前に反映する。運用・承認・検証限界は`NATION_RAID_FORMAL_RELEASE.md`を参照する。

- 国家対抗レイド1位称号・マーク（2026-09-25裁定）: 個人累計1位はヴァルグレイド「黒天竜討滅の覇者」、アストラギア「天墜機神討滅の覇者」、1行動最大damage1位はヴァルグレイド「黒天竜穿ちの極撃」、アストラギア「天墜機神砕きの極撃」とする。初回各1位の受取済み冒険者へ対応するヴァルグレイド称号を追加し、旧「万軍の先鋒」「天穿の一撃」と保存済み報酬snapshotは維持する。街ヘッダでは有効出撃条件を満たす同率を含む個人累計現在1位へ共通王冠、1行動最大damage現在1位へ剣と衝撃の専用マークを表示し、各最終1位は次回実開始まで保持する。次回開始後は現開催の該当者だけへ切り替え、条件未達1位から2位への繰上げはせず、六英雄冠と併記する。
- 国家対抗レイド1行動最大damage 2〜3位称号（2026-09-26裁定）: 有効出撃15回を満たす最終competition rank 2〜3位へ、ヴァルグレイド「黒天竜穿ちの剛撃」、アストラギア「天墜機神砕きの剛撃」を終了確定時の本人受取報酬として追加する。同率は全員対象、条件未達者からの繰上げはしない。能力補正、冒険者カードbadge、街ヘッダの専用マークは付けない。初回ヴァルグレイドの該当者には検証済み最終順位snapshotから受取済み報酬として遡及付与し、既存報酬snapshotは書き換えない。
...
- 1Characterは1国家。実効定員は国家Lv由来20〜40人と`nation.max_members`緊急上限の小さい方を使う。満員かつ募集ONでは加入申請を`waitlisted`で保持し、申請順の位置を表示するが、枠予約・自動加入・FIFOの承認強制は行わない。空きができた後に統治者が手動承認し、既存のtransaction/row lock付きServiceが定員を再確認する。加入申請・定員待ち登録時は統治者へ申請一覧へのURL付き通知、承認時は申請者へ未読通知を作る。加入・脱退・追放・再建国の待機時間は`nation.*` GameSettingを正本とする。解散待機中は一般国民の自主脱退を拒否し、解散完了時にcooldownなしで無所属へ戻す
- 国家チャットは所属dashboardと常設`ChatLog`の国家タブで同じ`nation_chat_messages`を使う。現在所属する同一国家の国民だけが100文字以内で送信し、最新50件を閲覧できる。所属dashboardは最新3件だけを表示し、全件はmodalで確認する。ほかの国民の新着がある間は下部国家ナビへ赤い未読dotを表示し、国家タブまたは常設チャットの国家タブを開くとmembership単位の最終既読message IDを更新する。加入前の履歴と自分の送信は未読にしない。全体/個人チャットの`public_logs`へは流さず、脱退・追放・解散完了後は閲覧・送信できない。所属国家TOPは横長ヘッダ直下によく使う操作を2列で置き、紹介・共同目標・募集素材・実績・最新年表を「国家概要」へ集約する。ruler向け国家操作履歴はdashboardに最新2件だけを表示し、過去分はmodalで直近100件まで確認する。統治者・国家発展・準備中機能のメニューは「全メニュー」modalへ集約するが、各機能の既存flagと権限gateは変更しない。冒険者カードは「所属国家」に正式国家名を表示し、無所属者は「無所属」と表示する。ただし非表示国家は所属本人のカードだけに正式名を表示し、他人からは「無所属」と表示する。2026-08-26にMariaDB 10.5.13の未読state migrationと既存membership backfillを本番適用し、不一致0で公開した
- 国家発展は`NATION_DEVELOPMENT_ENABLED`でcommunity/warから分離する。ON時は現在国民が40都市素材を画像付きtile modalから複数種類・個数指定で選び、確認modalを経て一括納品できる。素材一覧は所持数が多い順を既定とし、国家募集中優先・素材名順へ切替可能。納品数は－／＋と数量欄への直接入力に対応し、入力時は既存値を全選択して置き換える。低位素材は1個=国家資材1pt/国家発展EXP1、高位素材は1個=3pt/2EXPで、一括分の在庫減算・国家加算・台帳記録を同一transactionで確定する。国家Lvは累計EXPから整数閾値走査で導出し、次Lv必要EXPは`500×現在Lv`、表示上限Lv50（累計612,500）だが生EXPは上限後も保持する。国家外にはLvだけ、国民にはEXP進捗・現在国での個人貢献・退会者分を匿名集約した貢献一覧を表示する。`nations.development_exp`は台帳合計の読取cacheで、`nation:audit-development`が照合し、明示`--repair`時だけ復元する。発展実績の記録後は通常の`migrate:rollback`を実行せず、障害時はflag OFFと履歴を保持するforward migrationで復旧する。同一release batchの入口migrationも実績を検知して部分rollbackを拒否する。2026-08-26にMariaDB 10.5.13のstaging migration・schema/index・40換算率・shared lock readbackを通過し、本番はdevelopment flag ON、war flag OFFで公開した。MariaDBでの並行納品競合は未確認
- 国家Lv特典は定員とその他機能を分離する。Lv由来の実効定員はLv1の20人からLv50の40人まで段階開放し、`nation.max_members`は初期100人の緊急上限として常時適用する。Lvごとの定員表は未所属TOP・自国TOP・他国詳細の折りたたみ案内から確認できる。施設Lv上限、共同目標、募集素材、実績展示、装飾、納品分析、国家年表、戦争準備プリセットは`NATION_LEVEL_BENEFITS_ENABLED=false`で本番非公開とし、案内上も準備中と明記する。国家納品とLv・貢献表示は従来どおり利用できる。
...
- 「現在の六英雄」では、閲覧Character本人が首位かつ4ポーズ対応の場合だけ、そのキャラ画像とRoom名・名前・在位日数の枠をタップするとAlpine上でその画面内だけポーズを順送りする。他人のキャラ画像・情報枠と1ポーズ画像は冒険者カードを開く。DB上のアイコン・保存展示ポーズ・六英雄の競技データは更新しない
- Room色は封魔=紫・封刃=赤・灼命=橙・神速=緑・逆刻=青・奇跡=黄。六部屋図・Room選択・現在首位王冠で共通利用し、街ヘッダの「直近5分の全冒険者」では各Roomのrank 1へ対応色王冠を表示する。複数Room首位は王冠をenum定義順に並べる

## Job-art / exploration live metrics

- 管理者専用`/admin/gameplay-analytics`は、導入後の通常プレイヤー実績だけを記録する。戦技詳細は`gameplay_metrics`のversion 3生ログと、戦闘時間・経路・戦闘開始時の職業/Lv帯・順序付き構成で分けた時間単位DBロールアップへ同時に加算し、管理画面はロールアップの`SUM/GROUP BY`で集計する。現在のCharacter状態を過去戦闘へ後付けしない。管理者・`tester_%@valzeria.local`は記録・表示から除外し、旧戦技ログは保持するがスナップショットを推測で補完せず詳細集計に合算しない。dynamic-singleの抽選は候補戦技・実効発動率・実抽選値・戦闘時の職/戦技系譜をbattle-memoryへ記録し、同系譜不発の不足ptを時間単位集計する。管理画面の仮加算は観測済み抽選への一次試算だけで、ゲーム内発動率・SP消費・候補順・勝敗を変更または再計算しない
- 戦技は通常探索・追加ダンジョン・ボス・共有サブエリア・探索の地図・塔・英雄試練・プレイヤー闘技場（六英雄の公式戦を含む）・チャンプ戦・NPCランク戦の各実戦について、挑戦側冒険者の発動戦数、戦技別発動数、HIT/MISS/EVADE、HIT中の急所命中、勝敗、平均ターン、戦技によるHP/SP実回復量を記録する。訓練所・六英雄の相性確認・管理者ベンチマークは実績に含めない
...
- 戦技の表示威力は、各戦闘経路の通常攻撃100%相当を基準に線形適用する。通常探索・ボス・塔は各PvE式、チャンプ戦は通常攻撃と同じDuel式を維持する。チャンプ戦を除くPvP・NPCランク戦は、物理なら`E=防御`、魔力なら`E=精神`、攻撃側能力を`A`として`R=0.56A-0.30E+0.16×max(0,A-E)`、`M=0.18A×min(1,1.267A÷max(1,E))`、`Base=max(1,R,M)`を100%相当値とする。最大HP比例の最低ダメージとダメージ上限は使わない
...
- 戦技出力は本番公開済みで、通常/ボス/PvPごとに`none/low/standard/high/max`を保存し、「ゲームに任せる／優先順を決める」と独立して使う。戦闘開始時の装備込み最大SP `M0` を参照し、割引後の固定SPへRank別追加消費を足す。追加消費率はRank1=`0/0.25/0.75/1.50/2.50%`、Rank5=`0/0.50/1.50/3.00/5.00%`、Rank9=`0/0.75/2.25/4.50/7.50%`。最大SP10,000時はカード固有runtimeを含む基準powerが1以上の直接damageだけを`0/5/10/15/20%`強化し、基準power 0は固定費だけとする。10,000超は逓減、MAXは最終+30%を上限とする。SP回復・HP→SP変換戦技は対象外で、回復・軽減・強化・弱体・浄化・報酬・吸収回復量を増幅しない。対象となる非永続対戦の追加消費予算は`M0×25%`、不足時は候補を拒否し下位出力へ自動変更しない。探索予測用の対enemy訓練は通常探索と同じく予算なし、対人模擬戦は予算あり。出力の全依存flagが揃わない場合は保存POSTも拒否し、通常POSTはPRGで画面へ戻す。プレビューも現在職サポートを含むruntime同等gateでfail-closedする。詳細戦術は`BATTLE_JOB_ART_DETAILED_STRATEGY=false`の独立flag配下とし、OFF時は保存済み戦術を候補順・奥義発動率へ適用せず、従来設定を維持する。Rank5 v6 flagとは独立して利用できることを回帰テストで固定し、282戦技のSP出力除外47件は自然キー単位で監査する
- 戦技セット画面の各セット済み戦技カードに、選択中のSP出力に対応する戦闘前の基準消費SPを「固定SP＋追加SP」の内訳付きで表示する。出力選択を変えた時点でカード表示も切り替え、保存失敗時は選択値と表示値を同時に戻す。SP出力対象外の戦技は固定SPだけを表示する。戦闘中の固定費割引後に使った実値は戦闘ログを正とする。2026-08-30に本番公開

- チャンプ戦の戦技出力も本番公開済み。挑戦者・チャンプ双方がPvPの`sp_output`設定を使い、それぞれ戦闘開始時の装備込み最大SP `M0` に対する25%の戦闘内追加消費予算を持つ。チャンプの残SPは従来どおり防衛間で永続するが、この予算は各戦闘で作り直し、残量を次の防衛へ持ち越さない。緊急停止用の専用flagは残す。
...
- `config/battle.php`の戦技v2関連flagはコード既定OFF。本番では公開済みのv2基盤・UI・文言flag、`BATTLE_JOB_ART_RANK5_V6`、`BATTLE_JOB_ART_SP_POWER_SCALING`、`BATTLE_JOB_ART_SP_POWER_SCALING_CHAMP`をONにし、`BATTLE_JOB_ART_C_DESIGN_PROTOTYPE`と`BATTLE_JOB_ART_DETAILED_STRATEGY`はOFFを維持する。2026-08-29に戦技出力schema migrationとRank5 v6.1のmaster migration 2本を本番適用し、Rank5連携と戦技出力を公開した。実効flag、94件のRank5 master、戦技正本、公開HTTP、認証後の戦技セット画面を実機読戻しする。戦技出力を止める場合は主flagとチャンプ専用flagをOFFへ戻してconfig cacheを再生成する
...
- レイド専用戦技コンテキストは2026-09-25に本番公開。`raid`は5枠・SP方針・9種類のslot条件・おまかせ/こだわり設定・戦技出力を独立保存し、マイ戦技プリセットと公式プリセットの適用対象に含める。正式国家レイド、ローカル試遊、将来の国家レイドはすべて`raid`を読み、内部snapshot/作戦の互換キー`boss_set`は変更しない。移行時はraid行がないキャラクターだけbossの枠とcontext settingsをコピーし、既存raidデータを上書きしない。管理者の戦技採用率画面もraidを選択可能にする。
- 国家プロフィールは公開用の`join_policy`（加入方針、100文字以内）を持つ。募集ONの国家は満員でも加入申請を`waitlisted`として1件だけ保持し、申請順の位置を表示する。定員待ちは枠予約・自動加入・FIFO強制を行わず、空きができた後に統治者が手動承認する。承認時の国家行ロックと実効定員再確認は維持する。
- 装備市場の宛先指定出品は`recipient_character_id`を保持する。NULLは従来の公開出品、値ありは出品者・宛先本人・管理者だけが閲覧でき、宛先本人だけが購入できる。第三者の市場一覧・個人商店・直URLには存在を出さない。宛先選択は街ヘッダーと共通の直近5分の公開冒険者を検索前から全員表示し、名前検索後は従来どおり一致候補へ切り替える。査定範囲、10%手数料、72時間の期限・再出品制限、同時出品上限は公開出品と共通である。
- 装備市場の国家限定出品は`nation_id_snapshot`を出品時に固定し、現在の所属者だけが閲覧・購入できる。出品者は所属変更後も閲覧・取消できる。旧コードで公開出品に見えないよう`recipient_character_id`へ出品者IDを併せて保存し、新コードは国家IDを優先して権限判定する。防具名のランク接頭辞は武器と同じ形式にし、既存出品も表示時に補う。
...
- 通常・ボス戦のPRG結果は原則として冒険者別トークンの一時キャッシュへ保存し、失敗時はセッションへフォールバックする。表示用の敵と解放エリアは保存前に配列化し、表示時は旧キャッシュ・セッションに残る不完全オブジェクトも復元する。戦闘・報酬の確定処理とは分離する。
- Player equipment performance: weapon, armor, and accessory fixed stats and legacy stored affix stats are migration-scaled once to version 2 (×8), except HP is corrected to ×4. Dynamically calculated engraving HP also uses the ×4 scale (rounding up), while its other stats remain ×8. Weapon STR/MAG use `round(B × (1.00 + W / 3000))`, so no weapon keeps 100% and weapon performance 2400 yields 180%; armor DEF/SPR use `B + max(floor(W / 8), round(B × W / 2400))`, so they never fall below the pre-scale direct addition. Accessories add their fixed values directly; their non-HP enhancement result is kept at exactly ×8.
...
- 英雄試練は追加コンテンツキー `hero_trials` の既定値をOFFとし、OFF中は英雄試練殿を探索一覧へ出さず、試練殿・挑戦・結果の直URLを閉じるが、達成済みの英雄職は神殿に残る。管理画面でONかつ期間内にした場合、冠位の証を得て神殿に冠位職が表示された冒険者だけに、魔王城ヴァルゼリアの探索一覧で英雄試練殿を表示する。冠位の証があれば個別試練の条件未達でも殿内へ入れ、公開済みの試練だけを確認できる。未公開の試練は名称・画像・説明を含めて表示しない。現在は暁・月蝕・星天・時環の4試練だけ挑戦導線を実装済み。出現済み試練でHP/SPが全快でない場合は、条件確認モーダルに必要な踏破・冠位職マスター・現在HP/SPを表示し、HP/SPだけが未達なら既存の宿代・救済・Gold台帳ルールのまま同画面から宿屋で回復できる。暁、月蝕、星天はHP/SP持ち越しの二連戦、時環は単体戦で、第1陣4試練は総合戦力35万を入場条件ではない共通目安とする。二連戦化後の月蝕・星天はローカル35万等比補正構成×500固定シードで勝率68.8%・62.4%を確認し、本番匿名構成による再測定は未実施
...
- 通常10系統の装飾品S→SS進化は秘境解放条件なしで利用でき、S装飾品1個・古代装飾片3個・8,000Gを使う。古代装飾片はフェルディアの人型・巨人の敵、輝く宝箱、新規生成の古代片付き探索の地図で入手できる。分岐装飾品の廃止レシピは再開しない。ほかの古代片も従来どおり敵ドロップとフェルディア13探索地の輝く宝箱から入手できる
- 薬屋の探索補助品16種はすべて1回の調合で1個完成する。フェルディアの輝く宝箱は通常素材2〜4個のうち1枠を地域代表素材、残りを地域内の通常素材とし、古代片1.0%は別枠で維持する

Recent key points:
- 冒険者支援パス30日は50輝石で公開し、購入時は即時発動せず30日利用券が所持品に入り、使用時に初めて発動/延長する。通常パスと独立したシルバーウィーク仕様延長パスは2026-09-24 00:00以降、105輝石で30日利用券を販売し、使用中は探索力45秒/1・上限+500となる。各パスの期限・最大90日延長判定・利用券を別管理する。旅立ちセットは実装を維持するが今回の公開対象外として非表示にする。
- 冒険者カードの六英雄戦績は、今期の間別順位から最上位の間名と順位を先頭に「神速 2位」のように表示する。要約では間名末尾の「の間」を省き、間別一覧では正式名称を残す。同順位なら既存の間の表示順で一つ選ぶ。表示は既存の六英雄公開flagに従い、総合順位は作らない。
- 売買不可の支援アイテム「経験の護符」は、1個使用するごとに通常探索の対象勝利数を50回加算し、その間の獲得経験値を25%増加する。敗北、経験値0、ボス、亜域、追加ダンジョン、秘境の主、探索の地図、塔、PvP、チャンプ、NPCランク戦、国家戦では発動も回数消費もしない。Lv255では使用できず、既存効果も保留する。通常のプレイヤー入手経路と国家戦報酬への接続は未実装で、管理画面の個別送付だけを検証用に残す。
- 亜域探索では薬草・回復薬・魔力水を所持分から各10個まで持ち込み、通常探索と同じ戦闘結果欄から使用できる。街から記録済み入口へ直接入った場合も、サーバー側の亜域戦闘結果にある探索元エリアを持込文脈として使い、クライアント指定のエリアIDは信用しない。探索中のログアウトは中断として探索度・連戦数・危険度・持込残数・選択中の探索回数を保持し、再ログイン後は同じ入口へ復帰する。明示帰還または敗北で亜域探索を終了する。
...
- 街のお知らせは街ヘッダの街背景画像内で、街名・「直近5分の全冒険者」の人数と連続した3行構成で表示する。直近5分に活動した公開対象の冒険者は、人数・名前とも20人で打ち切らず全員分を20秒キャッシュで表示する。管理画面で公開した先頭3件を `[1/3]` 形式で32秒かけて流し、`[3/3]` の後は切れ目なく `[1/3]` へ戻る。📢横の「一覧」からゲーム内の更新履歴モーダルを開く。公開中の項目がない時は従来の「「ヴァルゼリアの冒険者」β版稼働中！」だけを表示する。管理画面では表示中のお知らせを下書きと別枠にし、新しく「表示する」を押した項目を先頭へ追加したうえで、上下ボタンにより公開順を1件ずつ変更する。公開順は公開日より優先され、公開日は表示開始日の判定に使う。管理用更新サマリのプレイヤー向け最新50件は、`TownUpdateService` が非公開下書きとして自動取り込みする。管理者が編集した文言は再同期で上書きしない。不要な候補は完全削除し、削除済みキーを最大50件保持して再生成を防ぐ。
- キャラアイコン制作は追加コンテンツ管理上の既定値をONとし、冒険者メニューの「案内」から全冒険者がヒアリングシートを開いて自動下書き保存できる。新規制作は同一アカウントの全キャラクターを合算して原則2セットまでとし、限定セットの付与履歴（取り消し済みを含む）が2件に達した後は新しい下書きを作らない。既存の下書き・提出済み依頼は継続でき、管理者は依頼詳細から監査情報付きの追加受付を1件だけ許可できる。確認画面へ進むと依頼ごとに40輝石（無償優先）で制作申請する。提出後は管理人との非公開チャットで候補画像を調整でき、制作完了前は提出済み回答を追加消費なしで修正できる。回答修正は管理画面の依頼一覧へ未確認の「回答更新あり」として表示する。限定アイコンは1セット1キャラクターへ付与し、闘技場の共有展示ポーズとチャンプ本人限定のチャンプ画面内4ポーズ切替に対応する。他の閲覧者のチャンプカードには通常ポーズだけを表示する。限定4画像は正方形の96pxまたは128pxに対応し、新規制作分は128pxを推奨する（表示枠のCSSサイズは変えない）。
- 通常選択アイコンは001〜270を選択でき、`public/images/chara/poses/chara_NNN/` に通常・戦闘・勝利・敗北の画像がある場合、通常表示と戦闘結果で場面別に自動解決する。不足画像は通常ポーズ、通常ポーズもなければ従来の `public/images/chara/chara_NNN.webp` へフォールバックする。
- ホーム画面左上の自キャラクターアイコンは、4ポーズ対応時だけタップ可能にし、通常→勝利→戦闘→敗北のファイル番号順でその画面内だけ表示を切り替える。切替はAlpineのローカル状態だけで行い、`characters.icon_path`や闘技場の保存展示ポーズは更新しない。利用可能なポーズが1種類だけの場合は従来どおり静止画像として表示する。
...
- HP/SPが少ない時の「次やること」は、所持回復アイテム、無料補給、銀行Gold、救済宿泊、素材・装備売却、通常宿泊の順で、現在利用できる導線を案内する。薬草・回復薬・魔力水は街の補給所でも使用できるが、探索中は持ち込み上限を迂回できないよう戦闘結果からのみ使用する。宿代はLv20以下またはキャラクター作成から10日未満なら10G、それ以外はLv×10Gで、作成日時は転職でリセットしない。回復量、無料補給数、Gold報酬は変更しない。
...
- 地図院の手元一覧では、表示中の未調査地図を複数選択し、共通の地図院、地図ごとの調査費、合計調査費、手持ち／銀行の支払い内訳を確認して一括調査できる。全対象の所有権・未調査状態・既存登録を行ロック後に再検証し、Gold支払いと全件の調査登録を同一transactionで確定するため、1件でも対象外なら全件を変更しない。単体調査・調査価格・調査後の公開操作は従来どおり。
...
- 管理者は `/admin/published-maps` で、公開中地図の詳細に加え、`cities.map_institute_development` を正とする街別の地図院発展値累計と、`map_income_logs` に記録された積立回数・最終積立日時を確認できる。表示は読み取り専用で、発展値や入場料分配を変更しない。
...
- 管理画面共通ヘッダは、管理人スレッドで冒険者の返信が最新になっている人数を1分ごとに確認し、ログアウト横の通知ベルとユーザー調査メニューへ未対応件数を表示する。ユーザー調査メニューには新しい順に最大3名の冒険者名も表示し、4名以上は残数をまとめる。ベルを押すと新しい順に最大3件の冒険者名・返信内容・時刻をポップアップ表示し、各返信からユーザー個別調査へ移動できる。管理人が同じスレッドへ送信するか、ベル内のゲーム内確認モーダルから会話履歴を残したまま「対応済みにする」と未対応から外れる。対応済み後に冒険者が再返信した場合は再び未対応になり、問い合わせメール件数との合計は既存のfavicon・画面タイトルにも表示する。ユーザー個別調査では、管理者だけが選択した一般ユーザーの冒険者へセッションを切り替えてゲーム画面を確認できる。管理者アカウントは対象外とし、切り替え時に管理者・対象ユーザー・冒険者を運用ログへ記録する。
...
- 装備強化はランク上限制（G〜E=+10、D〜B=+15、A=+20、S=+25、SS〜EPIC=+30）。武器・防具は+5まで各3%、以降は2%/1.5%/1.2%/1%/0.8%へ逓減して+30で合計47.5%上昇する。装飾品は正の基礎能力値比率を目安に強化分を1点ずつ累積配分する。S以下は+1〜+5で各段階合計+2、+6〜+30で各段階合計+1。SS/SSS/EPICは+30時に単能力型の能力値合計200/300/400、全能力型の各能力100/150/200へ到達する7月の目標値を使う。HP/SPを持つ装飾品は各正能力値に武器・防具と同じ段階別倍率を下限として適用し、HP/SPを持たない装飾品にはこの倍率下限を適用しない。毎段階で少なくとも一つの能力が増え、強化済み能力は下がらない。S以上のHP/SPを持たない単能力型11系統の+0基礎能力はS 208、SS 288、SSS 384、EPIC 520（時渡りの羽飾りSSSのみ敏捷376）。HP/SP型・複合型・全能力型・G〜Aの+0基礎能力は変更しない。進化では選択した進化元個体の+値を引き継ぐ。+6以降は石・高純度石・街素材・精錬核を段階的に消費し、成功率は100%。必要Goldは種類・ランクによらず到達強化値nに対してn×n×300G。市場の装備本体査定も+30まで同じ性能倍率を反映する。鍛冶屋の候補は銘・特攻・必要素材を一括取得し、種別ごとに20件ずつ段階表示する。
...
- 称号は進行・所持装備に加え、ヴァルゼリア通常70エリア・街道9エリアとフェルディア18探索地ごとに、対象印全種類を1個以上／各15個以上集めた段階で解放する。印称号は各街の通常7エリア、対応街道、次の街の順に表示する。地域踏破称号は各街の通常7エリア、全域踏破称号は通常エリア1〜70だけをボス撃破条件とし、街道・裏ダンジョン・英雄試練・フェルディアは条件外とする。既存地域・全域称号ID 4〜14と実績称号ID 112〜307・312〜329は、公開時に保存済み実績から冪等に遡及付与し、ホームで同時に複数獲得した場合は名称を連結せず、獲得件数を1件のメッセージにまとめる。ID 308〜311の国家レイド功績称号は変更しない。
...
- 探索力上限は勝利数で250→350(1000勝)→450(2000勝)→500(3000勝)。探索力制はOFF初期・運営切替。探索力制ON時の通常探索と亜域探索は1回/10回のクイック選択または2〜50回の回数指定を利用でき、同じ連戦集計処理で選択回数分の探索力を1戦ずつ消費する。まとめ探索は選択回数分の探索力がそろっている場合だけ開始し、敗北・HP30%以下・特殊イベント・深度入口到達・エラーで途中停止する（特殊イベント・深度入口は通常探索だけ）。回数選択は同じ探索中に保持し、戦利品を持って帰ると1回へ戻す。通常探索の結果は冒険者別の一時識別子でも保持し、チャット等の並行通信でセッション結果が古くなっても完了した結果を表示する。結果を確認できない通信では同じ探索POSTを自動再送しない。探索の地図は従来の×1/×10仕様を維持する
...
- チャンプ戦の素材報酬は装備中の進化レシピと共通素材から候補を作るが、素材マスタの`rank_tier`が4以上なら候補から除外する。古代装飾片・原初装飾晶・星屑の宝材は出ず、低位素材と既存所持品は維持する。星屑の宝材は代替入手経路が未実装のため、新規入手を一時停止し、進化画面では入手方法を「準備中」と案内する。
...
- `BATTLE_JOB_ART_HIT_RESOLUTION=true` はdynamic-singleとの両flag ONかつ現在職がprototype対応40職の時だけ、信頼済み分類のdamage戦技を共通`ActionResolver`でHIT/MISS/EVADEへ解決する。既定OFFで、非damage・分類不能戦技と対象外職はlegacy経路を維持する。accuracy未指定の既存戦技はPvE・ボス・塔でlegacyの1Hit命中率、NPC arenaでlegacyの基礎必中を再利用する。プレイヤーPvPとchampでは通常攻撃・damage戦技とも基礎90%、敏捷差係数0.08/0.15、下限84%・通常上限97%を使い、照準系damage戦技だけ上限99%。照準のraw命中率100%超過分とカード固有会心補正を急所命中率へ加え、PvP 12%・champ 20%で抑え、命中・能動回避後に1行動1回だけ抽選する。成立時は既存対人会心と同じ1.50倍を全Hitへ適用する。明示必中はraw超過と急所を作らず、NPC arenaは急所対象外。Rank5 v6の命中・会心値と黒焔魔皇破MISS後の次照準+25は構造化metadataから解決する。能動回避providerの本番既定値は0%。2026-08-30に本番公開
- 封狩の標的印による能動回避がEVADEとして成立した時は、回避したactorが攻撃者へ付与した標的印だけを1段階消費する。基礎MISS、回避抽選不成立、通常攻撃、非ダメージ戦技では消費せず、他のactorが付与した標的印にも触れない。共通`ActionResolver`を通る通常PvE・boss・tower・PvP・champ・NPC arenaのダメージ戦技へ同じ規則を適用する
...
- `BATTLE_JOB_ART_RANK5_V6=true` は上記resourcesまでの依存flagが成立した時だけ、94職のRank5 v6.1を有効にする。コード既定とenv exampleはOFF。scheduled連携は同resource内の固定slot順を詰めずに `4 × n` ptで解禁し、発動抽選の成否を問わずbattle-memoryのused状態へ入る。抽選不発は選択時点でusedを記録し、抽選成功は実行可否の最終検査を通過して実行へ確定した時点で記録するため、同じ成功行動を`blocked_by_rank5_cycle`で自己拒否しない。reactive連携はこの採番に含めず、カード固有最小値または4ptの大きい方を要求する。Rank5は資源を消費しない。同resourceのRank9を装備している場合はそのRank9成立時だけusedを解除する。同resourceのRank9がない場合は、resource上限12かつ4/8/12で解禁可能なscheduledがすべてusedになると、そのresourceとusedだけを0・未使用へ戻して自然循環する。未使用scheduledが残る間は12で待機し、reactiveはscheduledの完了を妨げない。reactiveだけのresourceは、いずれか1枚がusedかつ12到達で自然循環する。必要量16以上の4枚目以降は発動不能・自然循環の完了条件外とし、編成診断でerrorにする。攻撃なしは7/12/23/25/38/47の6枚だけで、他88枚は正本powerの攻撃を解決する。最終裁定として7は精神150%回復、10は威力100%と最大HP7%回復を行い、両者は次のdirect attackを1回20%軽減する。11は受け流し成功後の次のdirect attackを1回20%軽減し、15は通常20%・奥義/大技予告時は通常分と置換して35%、29と36は次の自分の行動開始まで20%軽減する。既に20%以上の軽減を持つカードと全94枚のpowerは変更しない。47は攻撃なしで精神120%回復・最大SP8%回復・Gold10%・通常素材8pt・レア素材5pt、84は威力244%の魔法damage・Gold2%・通常素材2pt・直前に上書きされた自分の場の5ラウンド再展開を併用する。独立レビューで見つかった47の回復不要時候補除外、29の表示・実行威力110%、成功Rank5の自己拒否を修正した。通常PvE・boss・towerでは、SP消費の確定成功後・減算前にusedを記録して同周期の再発動を防ぎ、SP消費の確定に失敗した場合はusedにしない。ステージングではMariaDB 10.5.13の94件migration `up/down/up`、ID維持、全依存flag ON、公開HTTP、認証後の戦技セット画面をPC/スマホ幅で確認した。通常PvE・boss・towerは実画面、PvP・champ・NPC arenaは同じステージングDBと本番Serviceを使うrollback付き非永続実行で、全6経路のRank5発動・戦闘ログ説明・HUDを確認した。flag ON時は依存flagと新master 94件をrelease readinessで照合し、MariaDBのDECIMAL返却値もJSON正本と数値比較する。通常releaseとステージング専用ZIP releaseはいずれもmigration前preflightと適用後のmaster検証・release readinessを必須とし、Rank5 master書換え許可は`maintenance_required`だけで渡す。旧ZIPクライアントはAPI契約version 2以上をHTTPSで確認できない限り`none`以外を送信せず、旧APIへの初回migrationを拒否する。`none`を含むserver API deployもmaster・readiness不一致時はfail closedする。2026-08-28に修正実装`2ec35026`を含む最新mainをステージングと本番へ同一SHA・`migration_mode=none`で反映し、master検証・release readiness・公開トップ/ログイン/manifestを確認した。2026-08-29にSHA`18cd298f`をステージングと本番へ同一SHA・`migration_mode=maintenance_required`で反映し、Rank5 migration 2本を適用、実効`rank5_v6=true`、依存flag、94件の新master、戦技正本、公開トップ/ログイン/manifest、認証後の戦技セット画面を読戻して本番公開した。2026-08-31の奥義なし自然循環はSHA`0ce78efe`をstaging・本番へ同一SHAかつ`migration_mode=none`で反映済み。SSH releaseは公開済み旧deploy API自体を更新しないため、同APIが契約headerを返すまでは旧ZIPのmigration付き送信をfail closedで拒否する
- `BATTLE_JOB_ART_FIELDS=true` はdynamic-single・hit-resolution・damage-application・resourcesとの全flag ONかつ参加者に対応済み場術職が含まれる時だけ有効。`BattleState` にDB非永続の主場1つ・副場1つとactor別の1ラウンドechoを保持し、主場は基本3ラウンド・最大5・1インスタンス1回だけ延長でき、生成/更新ラウンドは減算しない。信頼済み5種は星光（所有者の魔法ダメージ+10%）、旋律（所有者の奥義発動率+3pt）、聖域（所有者のHP回復+10%）、静寂（相手の資源獲得-1pt）、天測（所有者の命中+5pt・資源獲得+1pt）。本番生成は24 Rank1の聖域、46 Rank1の旋律、53/85 Rank1の星光、63 Rank1の固定5種cycle、現在職85 Rank9の1ラウンド旋律副場。53 Rank5は主場を+1、63 Rank5は上書きされた自分の旧場を1ラウンドecho、85 Rank5は主場がある時だけ星印4ptを消費して2ラウンド上書き不可にする。行動開始時スナップショットにより新しい場は同一行動へ自己適用しない。対人counterplayの封式の場で奥義を基礎効果だけ発動する場合も、旧DB master値ではなくCardDescription/CrownBalanceのL列正本値を実行用cloneへ適用し、source masterと抑止対象の動的固有効果は変更しない
- `BATTLE_JOB_ART_PENETRATION=true` はdynamic-single・hit-resolution・damage-application・resourcesとの全flag ONかつ正式metadataを持つ貫通系現在職32/45/52/62が自職由来の信頼済みRank5/9を使う時だけ有効。既定OFFで、物理DEF低減率は32=30%/50%、45=25%/40%、52=35%/50%、62=35%/50%。HIT後の既存ダメージ計算入力へ適用し、既存値と加算せず最大値だけを採用して絶対上限50%とする。SPR、Actorの永続DEF、障壁・軽減、PvPのminimum/floor/capは変更しない。構えは次項の追加flagで62だけに接続する。62 Rank9「竜冠天穿槍」の実行時威力はpenetration/stanceではなく`JobArtV2PowerResolver`が決め、同一戦闘で系譜を問わずRank5連携を1回以上使用済みなら470、未使用ならmaster 355を使う。使用済み判定はHIT/MISS/EVADE、現在職・継承を問わず、後段のrole-effect正本再適用でも条件成立時470を保持する
- `BATTLE_JOB_ART_PENETRATION_STANCE=true` は上記penetrationまでの全依存flag ONかつ現在職62の時だけ有効。既定OFFで、`BattleActor` にDB非永続・非スタック・期限なしの1チャージ構えを保持する。自職Rank1は発動時に付与し、自職Rank5は開始時snapshotに構えがあれば35%貫通へ使用した後、HIT/MISS/EVADEを問わず行動後に再付与する。自職Rank9は開始時snapshotに構えがあれば50%貫通へ使用し、結果を問わず再付与しない。構えなしでもRank5/9は使用できるがv2貫通はなく、構え単独の倍率・命中補正はない。Rank5は既存CT2・回数制限なしの反復可能consumer、Rank9は一戦一度。構え自体は威力倍率を加算せず、前項の355/470実行時威力条件とは独立する
