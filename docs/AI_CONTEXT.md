...
- 国家レイド事前案内（2026-09-05）: `NATION_COMPETITIVE_RAID_PREVIEW_ENABLED` は既定OFF。認証・キャラ選択済みGETのTOP/予定報酬/順位集計前案内だけを追加し、ホームのチャンプ下・国家TOP・全メニューから入れる。戦闘ボタンは準備中dialog。レイド本体・table・migration・開催Scheduler・受取POSTを含めない。表示用報酬snapshotは現行候補カタログから抽出したもので付与処理に使わない。公開状態は環境の実効設定と配備記録で確認する。正式レイドはこの案内版のflag切替だけでは開始できない。
