...
Recent key points:
- キャラアイコン制作は実装済みだが、追加コンテンツ管理上の既定値はOFF。OFF中は冒険者メニューから非表示にし、プレイヤー側のフォーム・確認・提出・専用チャット・添付画像の直URLを404で遮断する。管理画面の既存依頼と、付与済み限定アイコンの通常・戦闘・勝利・敗北表示はOFFの影響を受けない。ONにするとヒアリングシートの自動下書き・確認後に40輝石（無償優先）で提出でき、管理人との非公開チャットで候補画像を調整できる。限定アイコンは1セット1キャラクターへ付与し、闘技場の共有展示ポーズとチャンプ画面内の4ポーズ切替に対応する。限定4画像は正方形の96pxまたは128pxに対応し、新規制作分は128pxを推奨する（表示枠のCSSサイズは変えない）。
