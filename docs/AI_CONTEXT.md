...
- `BATTLE_JOB_ART_RANK5_V6=true` は上記resourcesまでの依存flagが成立した時だけ、94職のRank5 v6.1を有効にする。コード既定とenv exampleはOFF。scheduled連携は同resource内の固定slot順を詰めずに `4 × n` ptで解禁し、発動抽選の成否を問わず同じRank9成立までbattle-memoryのused状態へ入る。reactive連携はこの採番に含めず、カード固有最小値または4ptの大きい方を要求して同じ周期・解除規則を使う。Rank5は資源を消費せず、同resourceのRank9成立時だけusedを解除する。攻撃なしは7/12/23/25/38/47の6枚だけで、他88枚は正本powerの攻撃を解決する。最終裁定として7は精神150%回復、10は威力100%と最大HP7%回復を行い、両者は次のdirect attackを1回20%軽減する。11は受け流し成功後の次のdirect attackを1回20%軽減し、15は通常20%・奥義/大技予告時は通常分と置換して35%、29と36は次の自分の行動開始まで20%軽減する。既に20%以上の軽減を持つカードと全94枚のpowerは変更しない。47は攻撃なしで精神120%回復・最大SP8%回復・Gold10%・通常素材8pt・レア素材5pt、84は威力244%の魔法damage・Gold2%・通常素材2pt・直前に上書きされた自分の場の5ラウンド再展開を併用する。ステージングではMariaDB 10.5.13の94件migration `up/down/up`、ID維持、全依存flag ON、公開HTTPを確認済み。独立レビューで見つかった47の回復不要時候補除外と29の表示・実行威力110%を修正した。flag ON時は依存flagと新master 94件をrelease readinessで照合し、MariaDBのDECIMAL返却値もJSON正本と数値比較する。通常releaseとステージング専用ZIP releaseはいずれもmigration前preflightと適用後のmaster検証・release readinessを必須とし、Rank5 master書換え許可は`maintenance_required`だけで渡す。旧ZIPクライアントはAPI契約version 2以上をHTTPSで確認できない限り`none`以外を送信せず、旧APIへの初回migrationを拒否する。`none`を含むserver API deployもmaster・readiness不一致時はfail closedする。2026-08-28に`81add9bf`をステージングと本番へ同一SHA・`migration_mode=none`で反映し、master検証・release readiness・公開トップ/ログイン/manifestを確認した。本番はRank5 migrationが`Pending`、実効`rank5_v6=false`で新masterと新挙動を有効化していない。認証後実画面と6戦闘経路は未確認。SSH releaseは公開済み旧deploy API自体を更新しないため、同APIが契約headerを返すまでは旧ZIPのmigration付き送信をfail closedで拒否する
