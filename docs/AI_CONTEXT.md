...
- 冒険者カードは `AdventurerCardModal` の軽量Livewireインスタンスが表示イベントを受け、共通 `CityHeader` の能力値・通知・街情報を再描画せずにモーダルだけを更新する。クリック直後はクライアント側で読み込み表示を開き、職業バッジは圧縮した配列で受け取り、選択された階層だけ小さなLivewire更新で詳細データ・DOM・画像へ展開する。
...
Recent key points:
- 敵固有ドロップの武器14種は、進化元を含め開始ランクF〜Bを持ち、それぞれEPICまで既存武器へ合流しない専用一本道で進化する。進化先100武器は元武器の能力配分を武器ランク倍率で伸ばし、同武器種の現行進化素材を使う。進化元14種と進化先100種には全段階で個別の武器画像を表示する。火薬斧は高ATKとSPD低下、小鬼弓はLUK重視、葉隠れ弓はSPD重視として通常3分岐と役割を分ける。A以降は系統ごとに聖剣・魔剣・迅刃のいずれか1系統へ固定する。`DROP_WPN_*`はランクを持っても`DropService`の汎用抽選から明示除外し、敵別`enemy_drops`だけで入手する。元武器の銘・特攻・強化値は既存の進化処理どおり引き継ぐ。
