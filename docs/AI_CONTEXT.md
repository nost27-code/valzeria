...
- チャンプ戦の戦技出力は専用flagのまま既定OFFとする。ON時は挑戦者・チャンプ双方がPvPの`sp_output`設定を使い、それぞれ戦闘開始時の装備込み最大SP `M0` に対する25%の戦闘内追加消費予算を持つ。チャンプの残SPは従来どおり防衛間で永続するが、この予算は各戦闘で作り直し、残量を次の防衛へ持ち越さない。本番は未反映で関連flag OFFを維持する。
...
- `config/battle.php`の戦技v2関連flagはコード既定OFF。本番では公開済みのv2基盤・UI・文言flagと`BATTLE_JOB_ART_RANK5_V6`をONにし、`BATTLE_JOB_ART_C_DESIGN_PROTOTYPE`、`BATTLE_JOB_ART_SP_POWER_SCALING`、`BATTLE_JOB_ART_SP_POWER_SCALING_CHAMP`、`BATTLE_JOB_ART_DETAILED_STRATEGY`はOFFを維持する。2026-08-29に本番SHA`18cd298f`へ`maintenance_required`で更新し、戦技出力のschema migrationとRank5 v6.1のmaster migration 2本を適用した。実効flag、94件のRank5 master、戦技正本、公開HTTP、認証後の戦技セット画面を実機読戻し済み。戦技出力の新UI・追加SP消費・威力補正は引き続き非公開
