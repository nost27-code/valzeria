...
Recent key points:
- 探索の地図は、通常探索・通常ボス勝利時に低確率で生成される個別コンテンツ。`ExplorationMapGenerator` が秘匿root seedから等級・地図Lv・名称・接頭辞・探索上限に加え、7種類の報酬傾向から1つを永続化する。経験の導きはEXP+20%、金運上昇はGold額+50%、修練の導きはJ-EXP2倍（地図内だけ最大6）、素材探索は通常素材枠+5pt、装備の兆しは武器・防具+0.10pt/装飾+0.03pt、落銭の気配はGold抽選+3pt、精気の余韻は勝利時HP1.5%・SP0.5%回復。通常探索のJ-EXP上限3とGold抽選率は変えず、公開地図ではモンスター印を抽選しない。地図院では `MapSurveyService` / `MapPublicationService` が即時調査・公開を管理する。敵Lvは元の敵に対して通常+0〜5、希少+8〜15、英雄+20〜30、伝説+35〜50の4段階で決まり、上昇Lvを戦闘ステータスへ反映する。`MapExplorationRewardService` は補正後の実戦力に最も近い既存通常敵を基準に、経験値・Goldを最低10%上乗せする。地図傾向はその上に加算し、Goldの低確率抽選・乱数・職業補正は通常戦のまま維持する。公開地図の通常素材枠では、魔物の欠片・古びた徽章の候補重みだけを半分にして地域・敵固有素材を狙いやすくし、通常ダンジョンの素材候補重みは維持する。出現敵Lvには見習い級〜神話級の17段階の危険度名を付け、Lv240〜255の神話級では最大入場料を通常倍率の9倍にする。探索タブのストーリー導線の下にある「地図院で公開された地図」から公開地図一覧を開き、選んだ地図を探索する。共有探索は `MapExplorationBatchService` が登録行ロックとrequest UUIDで先に回数を予約し、結果を `map_exploration_batches` / `map_exploration_results` へ保存する。戦闘結果と継続操作は通常探索の `battle.result` を共用し、通常と同じ1回・×10探索で同じ地図を続けて探索する。入場料は街から地図へ入るたび1回だけ支払い、入場中の継続探索には追加でかからない。入場時には `MapExplorationItemService` が薬草・回復薬・魔力水を各10個まで持ち込み枠へ記録し、使った時だけ所持品から消費する。発見者70%・地図院20%・システム10%として `map_income_logs` に記録する。地図探索の勝利後も、通常探索と共通のヴァルモン卵抽選（当日上限・未所持種除外を含む）を行う。2026-07-29 23:59:59（日本時間）までは実装記念で地図ドロップ率を3倍にし、`game_settings` の `exploration_maps.launch_bonus_enabled` をOFFにすると途中停止できる。公開は72時間、上限は600〜2,000回とする。
- 公開地図一覧は、各地図をアコーディオンで表示して詳細を確認してから入場する。自分が直近に探索した地図を上に並べ、現在入場中の地図を明示する。地図院の手元一覧と公開地図一覧は、残り回数0または期限切れの地図を終了後6時間だけグレーアウトと赤い「終了」印付きで残し、入場操作を出さない。
- 公開地図一覧は広い画面では2列、スマホ幅では1列で表示する。各地図カードは探索地の種別に対応した背景画像を薄く敷き、地図詳細には出現敵Lvと、実際の地図補正後の敵ステータスから算出する目安戦力を表示する。
- 新規の探索地図は、入手元と同じダンジョンに固定せず、アークレア〜ヴァルゼリアの10都市の探索地系統を均等に抽選する。出現敵は系統と切り離し、地図等級の補正後にLv45〜140へ収まる中級〜上級帯から抽選する。生成済み地図は不変で、地図名候補は探索地系統ごとに拡充済み。
...
- 追加ダンジョンは `region_depth_dungeons` マスタと `RegionDepthDungeonService` を中心にした常設・地域別の継続探索です。管理画面で街・敵候補・強さ/EXP基準・入場料・危険度・EXP・Job EXPを設定して追加でき、街の探索一覧ではストーリーダンジョンの下に暗色カードで表示する。挑戦画面ではプレイヤー画像つきの個人最高記録と他プレイヤー上位10人の到達危険度ランキングを先に表示し、その下で探索する。探索中の危険度・連鎖・最高到達記録は専用テーブルで保持し、通常探索の深部・深層・最深層・異界層の入口・進行・補正を使わない。追加ダンジョンではモンスター印を抽選しない。
...
- 商店街は `/shopping-street`、自分の商店は `/shops/me`。素材・装備の既存市場出品は `shop_id` で個人商店と共有し、初回出品時に既定店舗を作る。保管済みのヴァルモン卵は `ShopEggListingService` が個体単位で12/24/48時間出品し、自己購入を拒否した上でGoldと所有者を行ロック内で移す。素材市場の成立時5%と装備市場の成立時10%、装備の72時間再出品制限は維持する。管理者は `/admin/player-shops` から営業状態を変更でき、停止中の店は新規出品・購入を拒否する。
- 銘・種族特攻は段階I〜Vを持つ。品質倍率は通常品1.00/良品1.15/逸品1.35、銘の基礎性能補正はI〜Vで8/16/24/32/40%、武器の種族特攻は6/12/18/24/30%に品質倍率を掛ける。銘は強化後の武器の最も高い基礎能力値を基準にし、生命の銘は算出値の3倍をHPへ加える。全能力を上げる調律の銘は、各能力の算出値を単能力銘の55%とする。装備ランク別の段階上限はG〜B=II、A=III、S=IV、SS以上=V。通常PvE・ボス戦にだけ特攻を反映し、PvP・チャンプ戦には反映しない。鍛冶屋の統合画面は「銘を鍛える」「特攻を鍛える」の二入口で、ベース・素材選択後に結果を自動判定する。同じ武器種・同じ特性・同じ段階なら段階を上げ、特性名が違うか素材の段階が高ければ武器種を問わず対象特性だけを移す。両方が一致する場合だけ、任意で「両方まとめて鍛える」を選べる。選択カードから既存の保護/保護解除も行え、完成後の武器名と必要Goldを実行前に確認できる。鍛錬画面の移し操作は、残す武器・消える素材武器・完成後の武器名・必要Goldを確認するモーダルで実行を選んで送信する。素材武器は消滅し、ベース武器の品質・強化値・装備/保護状態・市場再出品可能日時は維持する。
