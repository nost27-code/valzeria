...
- ホーム初期表示では `HomeActionPanel`・`LeftSidebar`・`ChampCard`・週間番付・`ChatLog` をon-loadの同一Livewireリクエストへ遅延し、固定高の読み込み表示後にまとめて置換する。`SchemaStateService` は同一リクエスト内のテーブル・カラム存在確認だけをメモ化し、探索・戦闘可否などのプレイヤー状態はキャッシュしない。
