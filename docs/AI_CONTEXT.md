...
- 装備市場の宛先指定出品は`recipient_character_id`を保持する。NULLは従来の公開出品、値ありは出品者・宛先本人・管理者だけが閲覧でき、宛先本人だけが購入できる。第三者の市場一覧・個人商店・直URLには存在を出さない。査定範囲、10%手数料、72時間の期限・再出品制限、同時出品上限は公開出品と共通である。
