...
- 装備強化はランク上限制（G〜E=+10、D〜B=+15、A=+20、S=+25、SS〜EPIC=+30）。武器・防具は+5まで各3%、以降は2%/1.5%/1.2%/1%/0.8%へ逓減して+30で合計47.5%上昇する。装飾品は正の基礎能力値比率を目安に追加値を1点ずつ累積配分し、+1〜+5は各段階で合計+2、+6〜+30は各段階で合計+1とする。S+25では配分値合計+30、SS〜EPIC+30では合計+35。HP/SPを持つ装飾品は各正能力値に武器・防具と同じ段階別倍率を下限として適用し、HP/SPを持たない装飾品は配分値を維持する。毎段階で少なくとも一つの能力が増え、強化済み能力は下がらない。SS以上の固定到達目標を使わない。S以上のHP/SPを持たない単能力型11系統の+0基礎能力はS 208、SS 288、SSS 384、EPIC 520（時渡りの羽飾りSSSのみ敏捷376）。HP/SP型・複合型・全能力型・G〜A、強化段階の追加値、進化では選択した進化元個体の+値の引き継ぎ、S→SSの解放条件は変更しない。+6以降は石・高純度石・街素材・精錬核を段階的に消費し、成功率は100%。必要Goldは種類・ランクによらず到達強化値nに対してn×n×300G。市場の装備本体査定も+30まで同じ性能倍率を反映する。鍛冶屋の候補は銘・特攻・必要素材を一括取得し、種別ごとに20件ずつ段階表示する。
