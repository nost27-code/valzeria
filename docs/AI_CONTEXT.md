...
- ホーム初期表示では `HomeActionPanel`・`LeftSidebar`・`ChampCard`・`ChatLog` をon-loadの同一Livewireリクエストへ遅延する。週間番付は30分ごとに先回り更新する取得時刻付きキャッシュを初期HTMLへ直接描画し、右上の更新ボタンでは全体10秒制限付きで最新集計へ更新できる。闘技場番付だけを表示後に取得する。期限超過時も最大6時間は直前表示を返しながらレスポンス後に更新する。各カードの配置順は維持する。`SchemaStateService` は同一リクエスト内のテーブル・カラム存在確認だけをメモ化し、探索・戦闘可否などのプレイヤー状態はキャッシュしない。
