...
- キャラアイコン制作の案内は追加コンテンツ管理上の既定値をONとし、冒険者メニューの「案内」から全冒険者がヒアリングシートを開いて自動下書き保存できる。制作受付の全体公開は既定OFFで、一般プレイヤーが「確認」を押すと入力途中でも下書きを保存し、「現在準備中です。もうしばらくお待ちください。下書き保存しました。」をゲーム内モーダルで表示する。確認画面・提出処理には進まず、40輝石も消費しない。`CHARACTER_ICON_DESIGN_PREVIEW_CHARACTER_IDS`で指定したキャラクターだけ確認・提出を先行検証でき、`CHARACTER_ICON_DESIGN_PUBLIC_ACCESS_ENABLED=true`で全体受付へ移行する。受付対象は確認後に40輝石（無償優先）で提出でき、管理人との非公開チャットで候補画像を調整できる。管理画面の既存依頼と付与済み限定アイコン表示は受付制限の影響を受けない。限定アイコンは1セット1キャラクターへ付与し、闘技場の共有展示ポーズとチャンプ画面内の4ポーズ切替に対応する。限定4画像は正方形の96pxまたは128pxに対応し、新規制作分は128pxを推奨する（表示枠のCSSサイズは変えない）。
