...
- v2の12pt経済校正では、既存Rank5を維持し、必要な現在職の信頼済みRank9だけを `JobArtV2PowerResolver` で実行時補正する。53=410、60=455、61=585、62=同一戦闘で自職Rank5使用済みなら470（未使用はmaster 355）、64=460、65=570、69=455。当時は66/67/68を固有効果込み比較でmaster 355維持としたが、67だけは2026-08-15の再裁定でmaster 315へ変更し、66/68は355を維持する。63はmaster powerを維持し、行動開始時の主場と本人の実上書き回数に基づく最大1.15倍だけを実行時に一度適用する。戦技セットの固定威力表示と実行時powerは同じResolverを使い、継承・対象外職・依存flag不成立時は既存マスタpowerへfail closedする。24/85と他のlegacy masterは変更せず、新しいpower専用flagは追加しない
