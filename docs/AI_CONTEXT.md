...
- 国家対抗レイド（2026-09-06）: 正式TOP・20再臨の戦闘・順位・履歴・報酬受取と管理者運用を実装。正式公開は`NATION_COMPETITIVE_RAID_ENABLED`（既定OFF）、案内は独立した`NATION_COMPETITIVE_RAID_PREVIEW_ENABLED`。旧ACTIVE keyは判定に使わない。開催は承認済みeventの状態で管理し、入口で自動作成しない。開始時の所属・装備・戦技を保存、探索力10、回数上限なし、7日間。初回のHPは第1〜4再臨各1000万、第5〜8各2000万、第9〜12各2億、第13〜16各5億、第17〜20各10億、総HP69億2000万。共闘ボーナスは2人+3%、3人+6%、4人+9%、5〜7人+12%、8〜11人+15%、12〜15人+17%、16〜18人+19%、19〜21人+21%、22人以上+22%。開催中の切替をまたぐ出撃は受付時の倍率を使う。初回だけ実時刻で予告短縮する専用CLIと、開催中の承認済みHP曲線・共闘段階を履歴やHPを維持して反映する専用CLIがある。報酬は終了後に明示確定して本人受取。運用・承認・検証限界は`NATION_RAID_FORMAL_RELEASE.md`。公開済みかは環境SHA/実効設定/DBを確認する。
