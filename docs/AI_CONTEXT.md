...
- 闘技場タブは `ArenaNpcRankingService::screenEntries()` でTOP6と挑戦候補3件だけを取得し、上位6名を1位／2〜3位／4〜6位の3段配置でキャラ画像中心に表示する。画面表示時に全ランキングの戦力を組み立てず、同一リクエスト内の順位整合性確認と倉庫集計も各1回にまとめる。
...
Recent key points:
- キャラアイコン制作の案内は追加コンテンツ管理上の既定値をONとし、冒険者メニューの「案内」へ表示する。制作受付の全体公開は既定OFFで、一般プレイヤーが項目を押すと「現在準備中です。もうしばらくお待ちください。」をゲーム内モーダルで表示し、フォーム・確認・提出・専用チャット・添付画像の直URLは404で遮断する。`CHARACTER_ICON_DESIGN_PREVIEW_CHARACTER_IDS`で指定したキャラクターだけ先行確認でき、`CHARACTER_ICON_DESIGN_PUBLIC_ACCESS_ENABLED=true`で全体受付へ移行する。受付対象はヒアリングシートの自動下書き・確認後に40輝石（無償優先）で提出でき、管理人との非公開チャットで候補画像を調整できる。管理画面の既存依頼と付与済み限定アイコン表示は受付制限の影響を受けない。限定アイコンは1セット1キャラクターへ付与し、闘技場の共有展示ポーズとチャンプ画面内の4ポーズ切替に対応する。限定4画像は正方形の96pxまたは128pxに対応し、新規制作分は128pxを推奨する（表示枠のCSSサイズは変えない）。
...
- 管理画面共通ヘッダは、管理人スレッドで冒険者の返信が最新になっている人数を1分ごとに確認し、ログアウト横の通知ベルとユーザー調査メニューへ未対応件数を表示する。ベルを押すと新しい順に最大3件の冒険者名・返信内容・時刻をポップアップ表示し、各返信からユーザー個別調査へ移動できる。管理人が同じスレッドへ送信するか、ベル内のゲーム内確認モーダルから会話履歴を残したまま「対応済みにする」と未対応から外れる。対応済み後に冒険者が再返信した場合は再び未対応になり、問い合わせメール件数との合計は既存のfavicon・画面タイトルにも表示する。
...
- 週間勝利数番付の毎日9:05の確定処理は、初回対象週から直前終了週までを確認し、season行が未作成の週も含めて未確定週を古い順に冪等回収する
