...
- 称号は進行・所持装備に加え、ヴァルゼリア通常70エリア・街道9エリアとフェルディア18探索地ごとに、対象印全種類を1個以上／各15個以上集めた段階で解放する。印称号は各街の通常7エリア、対応街道、次の街の順に表示する。地域踏破称号は各街の通常7エリア、全域踏破称号は通常エリア1〜70だけをボス撃破条件とし、街道・裏ダンジョン・英雄試練・フェルディアは条件外とする。既存地域・全域称号ID 4〜14と実績称号ID 112〜325は、公開時に保存済み実績から冪等に遡及付与し、ホームで同時に複数獲得した場合は名称を連結せず、獲得件数を1件のメッセージにまとめる。
