...
- 職業階層は8層（normal〜myth、EXP倍率1/2/5/8/10/15/22/30、転職引き継ぎ1/2・2/5・1/3）。冠位の証を所持すると冠位職が神殿に表示され、Lv30以上かつ未使用BPなしで超級職のマスターを問わず転職できる。証は討伐証として大事なものに分け、素材倉庫の保管数・種別数には含めない。ヴァルゼリア大陸の敵マスタは職業EXPを0〜3に収め、4以上を設定しない。職業EXPは各敵・モードの報酬設定または通常戦のLv差計算に従い、深度・亜域などの補正後も1回の報酬処理で最大3に制限する
