...
- 戦技セットの通常・ボス・PvP各セットには、runtimeと同じ有効資源判定による「有効な系譜」badgeを表示し、枠交換後は非同期で更新する。公式プリセット30件への導線は`battle.job_art_v2.official_preset_highlight_until`まで強調する。被物理・受け流し・実軽減による資源増加ログは計算時点を変えず、通常・ボス・塔・PvP・チャンプ・NPCランク戦でダメージ表示の直後へ送る。
...
- 探索力上限は勝利数で250→350(1000勝)→450(2000勝)→500(3000勝)。探索力制はOFF初期・運営切替。探索力制ON時の通常探索は1回/10回のクイック選択または2〜50回の回数指定を利用でき、まとめ探索は選択回数分の探索力がそろっている場合だけ開始し、敗北・HP30%以下・特殊イベント・深度入口到達・エラーで途中停止する。回数選択は同じ探索中に保持し、戦利品を持って帰ると1回へ戻す。探索の地図は従来の×1/×10仕様を維持する
