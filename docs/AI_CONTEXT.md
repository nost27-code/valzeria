...
- 戦技の表示威力は、各戦闘経路の通常攻撃100%相当を基準に線形適用する。通常探索・ボス・塔は各PvE式、プレイヤーPvP・NPCランク戦はランク戦式、チャンプ戦は通常攻撃と同じDuel式を使う。ランク戦式は100%相当値へ通常攻撃の下限・上限を適用してから表示倍率を掛け、戦技専用の1.85倍圧縮・総ダメージ35/40%上限・10%下限は使わない。通常攻撃の100%基礎値は維持し、将来の対戦バランス調整も表示倍率ではなく100%相当の通常攻撃式で行う
