...
Recent key points:
- 冒険者カードの「冒険の記録」は初期状態で閉じ、カードを開くリクエストでは集計しない。利用者が記録を最初に開いた時だけ専用 `AdventurerCardModal` が10分キャッシュ経由で戦闘・収集・育成記録を取得し、同じカードを開いている間は再取得しない
- 街のお知らせは街ヘッダの街背景画像内で、街名・現在の冒険者数と連続した3行構成で表示する。管理画面で公開した最新3件を `[1/3]` 形式で32秒かけて流し、`[3/3]` の後は切れ目なく `[1/3]` へ戻る。📢横の「一覧」からゲーム内の更新履歴モーダルを開く。公開中の項目がない時は従来の「「ヴァルゼリアの冒険者」β版稼働中！」だけを表示する。管理用更新サマリのプレイヤー向け最新50件は、`TownUpdateService` が非公開下書きとして自動取り込みする。管理者が編集した文言は再同期で上書きせず、掲載除外した候補も再生成しない。
- キャラアイコン制作の案内は追加コンテンツ管理上の既定値をONとし、冒険者メニューの「案内」から全冒険者がヒアリングシートを開いて自動下書き保存できる。制作受付の全体公開は既定OFFで、一般プレイヤーが「確認」を押すと入力途中でも下書きを保存し、「現在準備中です。もうしばらくお待ちください。下書き保存しました。」をゲーム内モーダルで表示する。確認画面・提出処理には進まず、40輝石も消費しない。`CHARACTER_ICON_DESIGN_PREVIEW_CHARACTER_IDS`で指定したキャラクターだけ確認・提出を先行検証でき、`CHARACTER_ICON_DESIGN_PUBLIC_ACCESS_ENABLED=true`で全体受付へ移行する。受付対象は確認後に40輝石（無償優先）で提出でき、管理人との非公開チャットで候補画像を調整できる。制作完了前は提出済み回答を追加消費なしで修正でき、管理画面の依頼一覧へ未確認の「回答更新あり」を表示する。管理画面の既存依頼と付与済み限定アイコン表示は受付制限の影響を受けない。限定アイコンは1セット1キャラクターへ付与し、闘技場の共有展示ポーズとチャンプ画面内の4ポーズ切替に対応する。限定4画像は正方形の96pxまたは128pxに対応し、新規制作分は128pxを推奨する（表示枠のCSSサイズは変えない）。
