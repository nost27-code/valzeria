...
- 国家対抗レイド（2026-09-05）: 正式TOP・20再臨の戦闘・順位・履歴・報酬受取と管理者運用を実装。正式公開は`NATION_COMPETITIVE_RAID_ENABLED`（既定OFF）、案内は独立した`NATION_COMPETITIVE_RAID_PREVIEW_ENABLED`。旧ACTIVE keyは判定に使わない。開催は承認済みeventの状態で管理し、入口で自動作成しない。開始時の所属・装備・戦技を保存、探索力10、回数上限なし、7日間、総HP6億。初回だけ実時刻で予告短縮する専用CLIを追加。報酬は終了後に明示確定して本人受取。運用・承認・検証限界は`NATION_RAID_FORMAL_RELEASE.md`。公開済みかは環境SHA/実効設定/DBを確認する。
