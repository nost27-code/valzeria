...
- 管理者専用`/admin/gameplay-analytics`は、導入後の通常プレイヤー実績だけを記録する。戦技詳細は`gameplay_metrics`のversion 3生ログと、戦闘時間・経路・戦闘開始時の職業/Lv帯・順序付き構成で分けた時間単位DBロールアップへ同時に加算し、管理画面はロールアップの`SUM/GROUP BY`で集計する。現在のCharacter状態を過去戦闘へ後付けしない。管理者・`tester_%@valzeria.local`は記録・表示から除外し、旧戦技ログは保持するがスナップショットを推測で補完せず詳細集計に合算しない
