...
## Main navigation / nation foundation（本番コード配備・公開OFF）

- home下部Navigationは街・探索・冒険者・商店街・国家・闘技場の6項目。`nation`は商店街と闘技場の間に常時表示し、`public/images/icon/icon_305.webp`の紋章を使う。`NATION_WAR_ENABLED=false`では従来の専用「準備中」パネルだけを表示する。明示ON時は`NationScreen`へ切り替え、建国、最大100人の所属、5施設、40都市素材の納品・国家資材台帳、国家/要塞/国民/戦争概要を表示する。OFF公開中は素材の入手可否も変更せず、`WEV0030`の敵dropは非活性を維持する
- 国家戦は専用tablesと`app/Services/Nation/`へ分離し、宣戦・予約・3日準備・5日戦争・敗戦保護、参加者snapshot、永続施設耐久、城壁lock、通常戦技を再利用する30T出撃、探索力/回数、魔導砲、修復/再建、判定/KO、戦史/資材返還を扱う。`nation_war.declaration_enabled=false`と`nation.facility_upgrades_enabled=false`が初期値で、`nation_war.reference_damage=0`（未校正）でも宣戦を拒否する
...
- 冒険者カードの「冒険の記録」は初期状態で閉じ、カードを開くリクエストでは集計しない。利用者が記録を最初に開いた時だけ専用 `AdventurerCardModal` が10分キャッシュ経由で戦闘・収集・育成記録を取得し、同じカードを開いている間は再取得しない。戦闘回数は実戦の勝利数+敗北数とし、旧形式の `victory` / `defeat` / `timeout` も読み替え、`turn_count=0` の非戦闘イベントは除外する。宝箱・秘境入口・亜域入口・ダンジョン主との遭遇では `characters.wins` も増やさない
