...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::table('items', function (Blueprint $table): void {
            $table->unsignedTinyInteger('accessory_performance_scale_version')->nullable()->after('armor_performance_scale_version');
        });
        ...
        DB::transaction(function (): void {
            $needsScale = fn ($query) => $query
                ->whereNull('accessory_performance_scale_version')
                ->orWhere('accessory_performance_scale_version', '<', self::SCALE_VERSION);
            ...
            DB::table('items')
                ...
                ->update([
                    ...
                    'accessory_performance_scale_version' => self::SCALE_VERSION,
                ]);

            DB::table('character_items')
                ->whereIn('item_id', DB::table('items')->where('type', 'accessory')->select('id'))
                ->where($needsScale)
                ->update([
                    // 旧方式で保存された銘補正だけを対象にする。動的な銘・強化は8倍化済みの本体性能から再計算される。
    ...
    public function down(): void
    {
        ...
        throw new RuntimeException('装飾品能力8倍化は不可逆です。リリース前バックアップから復旧してください。');
    }
};
