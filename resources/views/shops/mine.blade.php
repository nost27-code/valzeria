    ...
    <div class="mx-auto w-full max-w-3xl space-y-4 pb-10">
        ...
        <section class="rounded-xl border-2 border-slate-300 bg-white p-4 shadow-sm">
            ...
            <div class="mt-3 space-y-2">
                ...
                @foreach($equipmentListings as $listing)<div class="rounded-lg border border-slate-200 p-3 text-sm font-bold">装備：{{ $listing->display_name_snapshot }} ・ {{ number_format($listing->listing_price) }}G（{{ $listing->status }}）@if($listing->recipient_character_id)<span class="ml-1 text-amber-700">宛先：{{ $listing->recipient?->name ?? '指定した冒険者' }}さん</span>@endif</div>@endforeach
