<div class="w-full px-4 py-8 sm:px-6 lg:px-8 lg:py-10">
    <div class="mb-6 flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div>
            <p class="text-xs font-black tracking-[0.24em] text-rose-600">PLAYER FEEDBACK</p>
            <h1 class="mt-2 text-3xl font-black text-slate-950">ご意見・不具合</h1>
            <p class="mt-2 text-sm font-bold text-slate-500">ゲーム内から届いた改善の要望と不具合報告を新しい順に確認します。</p>
        </div>
        ...
        <aside class="overflow-hidden rounded-md bg-white shadow-sm ring-1 ring-slate-200">
            <div class="border-b border-slate-100 bg-slate-50 p-3">
                <div class="mb-2 flex flex-wrap gap-1.5">
                    @foreach(['all' => 'すべて', 'suggestion' => '改善の要望', 'bug' => '不具合'] as $key => $label)
                        <button type="button" wire:click="$set('kind', '{{ $key }}')" class="rounded-md px-2.5 py-1.5 text-[11px] font-black shadow-sm ring-1 transition {{ $kind === $key ? 'bg-amber-700 text-white ring-amber-700' : 'bg-white text-slate-600 ring-slate-200 hover:bg-slate-50' }}">
                            {{ $label }} {{ number_format($kindCounts[$key] ?? 0) }}
                        </button>
                    @endforeach
                </div>
            ...
            <div class="divide-y divide-slate-100">
                @forelse($reports as $report)
                    <button type="button" wire:click="selectReport({{ $report->id }})" class="block w-full px-4 py-3 text-left transition hover:bg-slate-50 {{ $selectedReport?->id === $report->id ? 'bg-rose-50' : 'bg-white' }}">
                        <div class="flex items-start justify-between gap-3">
                            <div class="min-w-0">
                                <div class="mb-1 text-[10px] font-black {{ $report->isSuggestion() ? 'text-amber-700' : 'text-rose-700' }}">{{ $report->kindLabel() }}</div>
                ...
                @empty
                    <div class="px-4 py-10 text-center text-sm font-bold text-slate-500">該当する送信はありません。</div>
                @endforelse
            </div>
        ...
        <section class="min-h-[30rem] rounded-md bg-white p-5 shadow-sm ring-1 ring-slate-200 sm:p-6">
            @if($selectedReport)
                <div class="flex flex-col gap-3 border-b border-slate-100 pb-4 sm:flex-row sm:items-start sm:justify-between">
                    <div>
                        <div class="text-xs font-black {{ $selectedReport->isSuggestion() ? 'text-amber-700' : 'text-rose-700' }}">{{ $selectedReport->kindLabel() }}</div>
                        <h2 class="mt-1 text-xl font-black text-slate-950">{{ $selectedReport->character?->name ?? 'キャラクター不明' }} の送信</h2>
                ...
                <p class="-mt-2 pb-5 text-xs font-semibold leading-relaxed text-slate-500">Codex用コピーで本文・報告元URL・利用環境をコピーできます。添付画像は必要なものを続けて貼り付けてください。</p>
                ...
                <div class="border-t border-slate-100 pt-5">
                    ...
                    @if($selectedReport->character)
                        ...
                        <form wire:submit.prevent="sendReply" class="mt-3">
                            <textarea wire:model="replyMessage" rows="4" maxlength="200" placeholder="この送信への返答を入力" class="w-full rounded-md border border-slate-300 bg-white px-3 py-2 text-sm font-semibold leading-relaxed text-slate-800 focus:border-rose-400 focus:ring focus:ring-rose-200"></textarea>
                ...
                <details class="mt-5 rounded-md border border-slate-200 bg-slate-50 p-3">
                    <summary class="cursor-pointer text-xs font-black text-slate-700">送信時の情報</summary>
            ...
            @else
                <div class="flex h-full items-center justify-center text-sm font-bold text-slate-500">左の一覧から送信内容を選択してください。</div>
            @endif
        </section>
    </div>
</div>
