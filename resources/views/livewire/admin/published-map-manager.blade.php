<div class="w-full px-4 py-8 sm:px-6 lg:px-8 lg:py-10">
    <div class="mx-auto max-w-7xl space-y-5">
        ...
        <section class="rounded-xl border border-amber-200 bg-white p-4 shadow-sm sm:p-5">
            <div class="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                <div>
                    <p class="text-xs font-black tracking-[0.18em] text-amber-700">MAP INSTITUTE DEVELOPMENT</p>
                    <h2 class="mt-1 text-lg font-black text-slate-950">地図院発展値（累計）</h2>
                    <p class="mt-1 text-sm font-bold leading-6 text-slate-600">ほかの冒険者が支払った入場料の20%分として、各地図院へ積み立てられた現在値です。引き出し可能なGoldではありません。</p>
                </div>
                <div class="grid shrink-0 grid-cols-3 gap-2 text-center sm:min-w-[25rem]">
                    <div class="rounded-lg bg-amber-50 px-3 py-2.5">
                        <div class="text-[11px] font-bold text-amber-800">全地図院合計</div>
                        <div class="mt-1 text-lg font-black text-amber-950">{{ number_format($totalInstituteDevelopment) }}</div>
                    </div>
                    <div class="rounded-lg bg-indigo-50 px-3 py-2.5">
                        <div class="text-[11px] font-bold text-indigo-800">積立あり</div>
                        <div class="mt-1 text-lg font-black text-indigo-950">{{ number_format($developedInstituteCount) }}<span class="ml-0.5 text-xs">院</span></div>
                    </div>
                    <div class="rounded-lg bg-emerald-50 px-3 py-2.5">
                        <div class="text-[11px] font-bold text-emerald-800">積立回数</div>
                        <div class="mt-1 text-lg font-black text-emerald-950">{{ number_format($totalContributionCount) }}<span class="ml-0.5 text-xs">回</span></div>
                    </div>
                </div>
            </div>

            <div class="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-3">
                @foreach($instituteDevelopments as $institute)
                    <article class="rounded-lg border border-slate-200 bg-slate-50 p-3.5">
                        <div class="flex items-start justify-between gap-3">
                            <h3 class="min-w-0 text-sm font-black leading-5 text-slate-900">{{ $institute['name'] }}地図院</h3>
                            @if($institute['development'] > 0)
                                <span class="shrink-0 rounded bg-amber-100 px-2 py-0.5 text-[10px] font-black text-amber-900">積立あり</span>
                            @else
                                <span class="shrink-0 rounded bg-slate-200 px-2 py-0.5 text-[10px] font-black text-slate-600">未積立</span>
                            @endif
                        </div>
                        <div class="mt-3 text-2xl font-black tabular-nums text-slate-950">{{ number_format($institute['development']) }}</div>
                        <div class="mt-2 flex flex-wrap items-center gap-x-3 gap-y-1 text-xs font-bold text-slate-600">
                            <span>積立 {{ number_format($institute['contribution_count']) }}回</span>
                            <span>最終積立 {{ $institute['last_contributed_at']?->format('Y/m/d H:i') ?? 'なし' }}</span>
                        </div>
                    </article>
                @endforeach
            </div>
