    ...
    <div class="flex flex-col md:flex-row gap-4 flex-grow overflow-hidden relative">
        ...
        <div class="w-full flex flex-col gap-0 rounded-xl overflow-hidden shadow-sm shrink-0 min-h-[80vh] {{ (!empty($isFerdiaRegion) || !empty($isFerdiaSimpleBase)) ? 'border border-emerald-300/70 bg-white' : 'bg-white border border-[#d4af37]' }}">
            ...
            <div class="p-4 flex-grow flex flex-col relative {{ (!empty($isFerdiaRegion) || !empty($isFerdiaSimpleBase)) ? 'bg-emerald-50/15' : 'bg-white' }}"
                ...
                @if($currentLocation === 'home' && !empty($departureSetHomeBanner))
                    <section class="mb-4 overflow-hidden rounded-xl border border-amber-300 bg-gradient-to-br from-amber-50 via-white to-sky-50 shadow-sm">
                        <div class="flex items-center gap-3 p-3">
                            @if(!empty($departureSetHomeBanner['icon_image']))
                                <img src="{{ asset('images/' . $departureSetHomeBanner['icon_image']) }}" alt="" class="h-12 w-12 shrink-0 rounded-lg bg-white p-1 object-contain shadow-sm">
                            @endif
                            <div class="min-w-0 flex-1">
                                <div class="text-[11px] font-black text-amber-700">はじめの冒険をまとめて支援</div>
                                <h2 class="mt-0.5 text-sm font-black leading-tight text-slate-900">{{ $departureSetHomeBanner['name'] }}</h2>
                                <p class="mt-1 text-[11px] font-bold leading-relaxed text-slate-600">支援パス、探索力の薬、倉庫拡張、限定カードフレームをまとめて受け取れます。</p>
                            </div>
                            <a href="{{ route('kiseki.support') }}#adventurer-departure-set"
                               class="shrink-0 rounded-lg bg-amber-600 px-3 py-2 text-center text-[11px] font-black text-white shadow-sm transition hover:bg-amber-700 active:scale-95">
                                <span class="block">{{ number_format($departureSetHomeBanner['price']) }}輝石</span>
