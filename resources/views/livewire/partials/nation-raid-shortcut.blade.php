...
<section class="overflow-hidden rounded-2xl border border-indigo-200 bg-white shadow-sm" data-nation-raid-shortcut data-nation-raid-entry-state="{{ $nationRaidPublished ? 'published' : 'preparing' }}">
    ...
    >
        <span class="flex h-12 w-12 shrink-0 items-center justify-center rounded-xl bg-slate-900 text-2xl shadow-sm" aria-hidden="true">⚙️</span>
        <span class="min-w-0 flex-1">
            <span class="flex flex-wrap items-center gap-2">
                ...
                @if($nationRaidPublished && !config('features.nation_competitive_raid_enabled', false))
                    ...
                    <span class="rounded-full bg-amber-50 px-2 py-0.5 text-[10px] font-black text-amber-800">{{ config('nation_raid_preview.starts_at_label') }}開始予定</span>
                @endif
            </span>
            <span class="mt-1 block text-xs font-bold leading-relaxed text-slate-600 sm:text-sm">全国家の冒険者でレイドボスへ挑む。無所属でも参加できます。</span>
        </span>
