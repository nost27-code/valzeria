...
<x-layouts.facility title="装備市場" headerIconImage="images/icon/icon_032.webp" bgImage="images/facilities/item.webp">
    <div class="mx-auto w-full max-w-xl pb-10"><a href="{{ route('equipment-market.index') }}" class="mb-3 inline-block text-sm font-black text-violet-700 hover:underline">← 装備市場へ戻る</a>
        <article class="rounded-lg border border-violet-200 bg-white p-5 shadow-sm"><div class="text-xs font-black tracking-wide text-violet-700">EQUIPMENT MARKET</div><div class="mt-1 flex items-start gap-2">@if($equipmentIcon)<img src="{{ asset($equipmentIcon) }}" alt="" class="mt-0.5 h-7 w-7 shrink-0 object-contain">@endif<h1 class="text-xl font-black text-slate-900">{{ $listing->display_name_snapshot }}</h1></div><div class="mt-2 flex flex-wrap gap-x-3 gap-y-1 text-sm font-bold text-slate-500"><span>出品者：{{ $listing->seller?->name ?? '不明' }}</span><span>種類：{{ $categoryLabels[$listing->weapon_category] ?? ($listing->weapon_category ?: '装備') }}</span><span>強化：{{ $listing->enhance_level > 0 ? '+' . $listing->enhance_level : 'なし' }}</span></div>
            @if($listing->recipient_character_id)<div class="mt-3 rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-sm font-black text-amber-900">{{ (int) $listing->recipient_character_id === (int) $character->id ? 'あなた宛ての出品です' : '宛先：'.($listing->recipient?->name ?? '指定した冒険者').'さん' }}</div>@endif
