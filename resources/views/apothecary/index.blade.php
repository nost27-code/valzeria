            ...
            <div class="mb-5 rounded-lg border {{ $activeSupport ? 'border-emerald-200 bg-emerald-50/50' : 'border-slate-200 bg-slate-50' }} p-3.5">
                ...
                @if($activeSupport)
                    <div class="mt-1.5 flex flex-wrap items-center justify-between gap-2">
                        ...
                        <span class="text-xs font-mono font-bold text-slate-600">残り {{ $activeSupport['remaining'] }}&thinsp;/&thinsp;{{ $activeSupport['max_battles'] ?? 50 }}戦</span>
                    </div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-2.5">
                @foreach($recipes as $recipe)
                    @php
                        ...
                        $remainingBattles = (int) ($recipe['remaining_battles'] ?? 0);
                        $isActiveSupport = $activeSupport && $activeSupport['item_key'] === $recipe['support_key'];
                        $canActivateSupport = $remainingBattles > 0 || $recipe['owned_item_count'] > 0;
                        $activateLabel = $remainingBattles > 0
                            ? "再装備（残り{$remainingBattles}戦）"
                            : ($isActiveSupport
                                ? '補充する（予備' . number_format($recipe['owned_item_count']) . '個）'
                    ...
                    <div class="rounded-lg border {{ $cardClass }} px-3 py-2.5 flex flex-col gap-2">
                        ...
                        <div class="flex items-center justify-between gap-2">
                            ...
                            @if($isActiveSupport && $remainingBattles > 0)
                                <span class="shrink-0 rounded border border-emerald-200 bg-emerald-50 px-2 py-1 text-[10px] font-black text-emerald-700 leading-none">
                                    使用中（残り{{ number_format($remainingBattles) }}戦・予備{{ number_format($recipe['owned_item_count']) }}個）
                                </span>
                            @else
                                <button
                                    type="button"
                                    @if($canActivateSupport)
                                        @click="activateSelected = {
                                            itemKey: @js($recipe['support_key']),
                                            name: @js($recipe['name']),
                                            actionLabel: @js($remainingBattles > 0 ? '再装備' : ($isActiveSupport ? '補充' : '使用')),
                                            remaining: {{ $remainingBattles }},
                                            maxBattles: {{ $activeSupport['max_battles'] ?? 50 }},
                                            consumesReserve: {{ $remainingBattles > 0 ? 'false' : 'true' }},
                                            switching: {{ $activeSupport && !$isActiveSupport ? 'true' : 'false' }}
                                        }; activateModalOpen = true"
                                    @else
                                        disabled
                                    @endif
                                    class="shrink-0 text-[10px] font-bold leading-none underline underline-offset-2 transition {{ $canActivateSupport ? 'text-emerald-700 decoration-emerald-300 hover:text-emerald-900' : 'text-slate-400 decoration-slate-300 cursor-not-allowed' }}"
                                >
                                    {{ $activateLabel }}
                                </button>
                            @endif
            ...
            <div class="flex min-h-screen items-center justify-center p-4 text-center">
                ...
                >
                    <form method="POST" action="{{ route('apothecary.activate') }}" x-data="{ submitting: false }" @submit="if (submitting) { $event.preventDefault(); return; } submitting = true">
                        ...
                        <div class="border-b border-slate-200 px-5 py-5">
                            <h3 class="text-lg font-extrabold text-slate-900" id="activate-modal-title">探索補助品の変更</h3>
                            <p class="mt-3 text-sm text-slate-600 leading-relaxed">
                                <span class="font-bold text-slate-900" x-text="activateSelected?.name"></span>
                                を<span x-text="activateSelected?.actionLabel || '使用'"></span>しますか？
                            </p>
                            <p class="mt-3 rounded bg-amber-50 px-3 py-2 text-xs font-bold text-amber-800" x-show="activateSelected?.switching">
                                装備を切り替えても、現在の品の残り戦数と発動回数は保存されます。
                            </p>
                            <p class="mt-2 rounded bg-orange-50 px-3 py-2 text-xs font-bold text-orange-800" x-show="activateSelected?.consumesReserve">
                                予備を1個消費し、<span x-text="activateSelected?.maxBattles || 50"></span>戦分を開始します。
                            </p>
                            <p class="mt-2 rounded bg-emerald-50 px-3 py-2 text-xs font-bold text-emerald-800" x-show="activateSelected && !activateSelected.consumesReserve">
                                予備は消費せず、残り<span x-text="activateSelected?.remaining || 0"></span>戦から再開します。
                            </p>
                        </div>
                        <div class="bg-slate-50 px-5 py-4 sm:flex sm:flex-row-reverse sm:gap-3">
                            <button type="submit" :disabled="submitting" class="inline-flex w-full items-center justify-center gap-2 rounded-md bg-emerald-600 px-4 py-2.5 text-sm font-bold text-white shadow-sm transition hover:bg-emerald-700 disabled:cursor-wait disabled:opacity-70 sm:w-auto">
                                <x-loading-spinner x-show="submitting" style="display: none;" />
                                <span x-show="!submitting" x-text="(activateSelected?.actionLabel || '使用') + 'する'"></span>
                ...
                >
                    <form method="POST" action="{{ route('apothecary.clear') }}" x-data="{ submitting: false }" @submit="if (submitting) { $event.preventDefault(); return; } submitting = true">
                        ...
                        <div class="border-b border-slate-200 px-5 py-5">
                            ...
                            <p class="mt-3 text-sm text-slate-600 leading-relaxed">
                                装備を外しても、残り戦数と発動回数は保存されます。解除しますか？
                            </p>
                        </div>
                        <div class="bg-slate-50 px-5 py-4 sm:flex sm:flex-row-reverse sm:gap-3">
