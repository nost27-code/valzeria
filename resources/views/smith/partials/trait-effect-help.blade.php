...
<section x-data="{ traitHelpTab: 'effects' }" class="{{ $traitEffectHelpMargin }} rounded-lg border border-violet-200 bg-violet-50 p-3">
    <h4 class="font-black text-violet-950">銘と特攻の効果</h4>
    <div class="mt-3 grid grid-cols-2 gap-2 rounded-lg border border-violet-100 bg-white p-1" role="tablist" aria-label="銘と特攻の解説">
        ...
        <button type="button" role="tab" @click="traitHelpTab = 'targets'" :aria-selected="traitHelpTab === 'targets'" :class="traitHelpTab === 'targets' ? 'bg-sky-700 text-white shadow-sm' : 'text-slate-600 hover:bg-sky-50'" class="rounded-md px-3 py-2 text-sm font-black transition">特攻一覧</button>
    </div>
    ...
    <div x-show="traitHelpTab === 'targets'" class="mt-3">
        ...
        <p class="mt-2 text-xs font-bold text-sky-800">特攻は通常探索・ボス戦で有効です。闘技場（PvP）・チャンプ戦には効きません。</p>
    </div>
</section>
