Create one horizontal animation strip for Codex pet `val`, state `running-right`.
...
Identity: same pet in every frame: A compact whole-body fantasy coding companion inspired by a nostalgic FFA browser RPG developer: warm, precise, mobile-friendly, carries a tiny lantern-like kiseki crystal and a small toolkit satchel, no readable text or logos, helpful and steady.. Preserve silhouette, face, proportions, markings, palette, material, style, and props.
Style: Pet-safe sprite: compact full-body mascot, readable in a 192x208 cell, clear silhouette, simple face, stable palette/materials, and crisp edges for chroma-key extraction. Style `sticker`: Polished sticker mascot with bold clean shapes, crisp outline, flat colors, and minimal highlight detail. User style notes: Clean sticker mascot, readable at 192x208, warm fantasy palette with jewel accents, simple silhouette, no text..
