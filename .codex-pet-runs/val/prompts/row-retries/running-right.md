Create Codex pet row `running-right` for `val`: exactly 8 full-body frames in one horizontal strip on flat pure magenta #FF00FF.

Use the attached canonical base for identity and the layout guide only for spacing. Same pet in every frame: A compact whole-body fantasy coding companion inspired by a nostalgic FFA browser RPG developer: warm, precise, mobile-friendly, carries a tiny lantern-like kiseki crystal and a small toolkit satchel, no readable text or logos, helpful and steady.. Preserve silhouette, face, palette, material, proportions, markings, and props.
